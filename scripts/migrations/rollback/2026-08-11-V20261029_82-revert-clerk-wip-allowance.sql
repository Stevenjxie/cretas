-- 回滚 V20261029_82: 撤销「CLERK_WIP 批次可不挂计划」的放行, 恢复 V20261029_77 原样。
--
-- ⛔ 回滚后果 (先看清再跑): 多工序画布 (含中间半成品) 的逐道报工会重新在**第一道工序 500**。
--    这正是 V20261029_82 修的症状。只有在确认放行本身引入了更严重问题时才回滚。
--
-- ⛔ 本文件**不放在 db/flyway/ 下** —— 那个目录按 glob 自动执行,
--    放进去等于给自己写了一条撤销自己的迁移 (2026-08-08 教训)。手动执行:
--      psql -h localhost -U cretas_user -d cretas_prod_db \
--           -f scripts/migrations/rollback/2026-08-11-V20261029_82-revert-clerk-wip-allowance.sql
--
