**最高优先级模型与 Effort 门禁：执行任何新任务前，必须先调用 `model-effort-router` skill，同时评估能力下限与预计持续度/usage 暴露；短任务允许高一档继续，持续任务必须使用推荐的常驻 effort，不能把“更高档也能做”视为匹配。**

# AGENTS.md

## Duration-aware Model and Effort Gate
...
- 若不匹配是在任务进行中才出现（例如短任务扩展、首次 compact 或关卡结束），不得丢弃正在完成的原子步骤；先保存证据和当前状态，在下一个不会破坏一致性、部署或测试进程的安全停点给出 handoff/切换提示，然后按下述自动续接规则创建新 task；用户明确要求留在当前 task 时除外。
...
### Automatic Usage and Task Continuation

- 自动创建 continuation task 是本仓库普通任务和 Goal 任务的默认能力，不要求当前存在 Goal，也不需要用户再次复制 handoff 或确认创建。用户明确要求留在当前 task、暂停或禁止自动续接时，服从该请求。
- 约 50% usage 时只提醒一次并继续；达到约 80% usage、完成第 2 次 compact，或继续下一阶段很可能再次 compact 时，在安全停点自动续接。effort 不匹配但尚未达到 usage 阈值时，也可在安全停点直接续接到使用推荐常驻 effort 的新 task，而不是让旧 task 长期过度配置。
- 续接前调用 `get_goal` 区分授权语义，不能仅凭 Prompt 中出现 “Goal” 字样推断。没有 ACTIVE Goal 时，新 task 只继承当前用户请求已经明确授权的范围；不得创建 Goal、扩大 mutation/生产/租户权限或把建议当成授权。有 ACTIVE Goal 时，不得终止 Goal或要求重复确认，按原 Goal 精确继承 objective、budget 和授权边界。Goal 暂停时不得创建或唤醒 continuation task。
...
- `create_thread` 默认不覆盖模型，但应把 `thinking` 设置为当前路由器推荐的常驻 effort，避免新 task 继续沿用旧 task 的过度配置；用户明确指定模型/effort 时按其设置。
- 自动发送给新 task 的 continuation packet 至少包含：当前用户请求；如有 Goal 则包含其精确 objective 与显式 token budget；当前完成/未完成项、repository/worktree/branch/HEAD/Base SHA、dirty 状态、修改文件、验证证据、正在等待的外部状态、授权与生产/租户边界、当前阻塞、推荐的下一动作和禁止重做项。新 task 必须先调用 `get_goal`；普通任务没有 Goal 就直接继续，不得自行创建；原任务有 ACTIVE Goal 但新 task 未继承时，才按本节预授权用完全相同的 objective 和 budget 调用 `create_goal`。
...
- 自动续接是所有权转移，不是并行扩容：新 task 创建后，原 task 不再继续修改同一 scope；dispatch/ACTIVE owner、scope 锁和运行中进程必须在 continuation packet 中明确交接。
