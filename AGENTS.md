...
4. 发布必须从 clean、exact local `main` 的专用 release worktree/快照执行，并锁定该本地 main SHA。若现有发布脚本仍硬性要求 `HEAD == origin/main`，不得手工删除或绕过保护；先使用/实现带显式确认、exact-local-main 校验和结构化回执的离线入口，再发布。
5. commit、合入本地 main、部署、迁移、服务验收和业务验收继续分开报告；“已合入本地 main”不等于已部署。
6. GitHub 恢复后，先只读比较 local main 与 `origin/main`，通过普通 merge/rebase 解决差异并推送，禁止 force push；恢复在线真值后自动回到本文件的常规 PR/fastlane 与 exact `origin/main` 发布规则。
...
**从 GitHub 取制品送上服务器**（`GitHub → 东京 Lightsail → 上海 OSS → ECS 内网`，制品字节完全不经过本机）由 `scripts/deploy/Publish-GitHubArtifactViaLightsailOss.ps1` 编排。⚠️ **2026-07-31 起它已是常规发布的默认路径，不再是旁路**（此处原写「不加速常规发布」，已随 PR#2061 作废）：`release-cretas.sh` 在 `--prefer-ci-artifact`（默认开）下调用 `release-ci-artifact.sh`，后者调用这个编排器复用 CI 已构建的 JAR，命中时 Java 侧**零次 Maven**（实测构建段 204s → 25s）；Web 侧同理走 `release-web-ci-artifact.sh`。命中的前提是制品已产出，所以合并后先跑 `prewarm-main-artifact.sh`（规则 11）。未命中才回退本地 Maven 构建 + rsync delta（那条路 delta 实测 9.6×、只传 ~20MB，而 OSS PUT 无 delta 必传全量 —— 两条路各自占优的场景不同，不要据此认为本地构建总是更快）。手工传 `-AssetId` / `-ArtifactId` 直接调 ps1 仍是排查与人工回退用途。输出里 `transport_verified` 与 `deployable_trust_verified` 是两件事，后者只在 manifest 的 tree 与测试选择器都对得上时才为 true。详见 `scripts/deploy/RELEASE-ARTIFACT-TRANSPORT.md`。
...
6. **受控无 PR 快速通道** - PR 仍是默认路径。只有用户明确要求“不做 PR/直接发 main”、本批只有一个协调者且已在 clean `codex/*` worktree 完成 scope 审查和目标验证时，才可使用 `scripts/deploy/publish-main-fastlane.sh` 快进推送。必须锁定登记的 Base SHA、在同一最终 commit 归档 ACTIVE 并释放 scope、推送前重新 fetch 且证明 `origin/main` 未前进；禁止 force push。非 docs 批次必须传准确的 `--task-id <本批任务 ID>`：ACTIVE 常驻数十条他人在飞任务，不传时门禁要求「全局零未完成任务」，该条件实际上永远不成立，会把每一次合法直推都拒成 PR；`--task-id` 只检查在飞区是否仍有匹配该 ID 的未完成行，使无关任务不再阻塞。脚本会要求该 ID 以词边界形式真实出现在 `docs/dispatch/`（ACTIVE 或归档），拼错或不存在的 ID 会被拒绝而非静默放行（PR #1962）；但脚本只能证明 ID 存在，不能证明归档内容正确，调用方仍须在同一 commit 归档自己的批次并释放 scope。脚本任一门禁失败即回退为一次 PR，不得手工绕过。迁移、Entity/Repository/Security、共享发布脚本等高风险 scope 默认仍走 PR；只有用户明确授权且必需深度门禁已通过时才可传入高风险覆盖口令。合入 main 与生产部署仍是两个独立状态。
...
1. **唯一发布构建与可信 manifest** - 后续发布必须从干净且 `HEAD == origin/main` 的 exact release worktree 执行。同一 `backend/java/cretas-api` Git tree 与目标测试选择器只允许零或一次 `mvn clean package -Dtest=<tests>` 生命周期：manifest 格式合法且 `success=true`、记录的 `target_tests` 与本次选择器完全一致、记录的 backend tree 等于当前 HEAD backend tree、缓存 JAR 存在且通过归档完整性与 SHA-256 校验时，`release-jar-manifest.sh build` 直接复用并运行零次 Maven；任一条件不满足才执行一次 clean package，`CRETAS_RELEASE_FORCE_JAVA_BUILD=1` 可显式强制该构建。不得先单跑目标测试再重复 package。部署阶段的 manifest/制品复用还必须证明记录的 build commit 可由 Git 解析，且该 commit 的后端 tree、manifest 记录的后端 tree、当前 `origin/main` 后端 tree 三者一致，同时通过 SHA-256、JAR 完整性和当前 worktree clean 校验。squash 后 commit 不同但后端 tree 完全相同可复用；任一校验失败只回退一次本地 `clean package`。生产当前 JAR 的 MD5、真实 upstream、active systemd 与直连健康均匹配时，Java 未变化可按后端 tree cache 判定安全 no-op。
...
6. **部署快速失败与制品复用** - Java 日常部署先检查 manifest-backed 后端 tree cache；Java 未变时允许 cache 命中或 no-op。GitHub Artifact **自 2026-07-31（PR#2061）起是默认优先路径**，不再是「显式手动备用」：`--prefer-ci-artifact` 默认开（关掉用 `--no-prefer-ci-artifact` 或显式导出 `CRETAS_RELEASE_PREFER_CI_ARTIFACT=0`，两个方向都要显式导出，否则会变成 Java 侧关而 Web 侧照样开）。它仍必须通过 exact-commit 清单、SHA-256 与部署阶段 JAR 完整性校验。**禁止为了等制品而触发 CI 后原地空等**；合并后要等的那一段用 `prewarm-main-artifact.sh --wait`（幂等、有界、可丢后台，见规则 11）承担，它只轮询 CI 已在构建的制品，不触发新的 CI 运行。缓存/Artifact 缺失或校验失败时只回退一次现有本地 `clean package`，不得再次 retry package。相同制品 no-op 仍必须读取真实 upstream 并验证 active systemd 与健康；真正切流时 5×6 秒观察和自动回滚不得省略。idle 槽连续自动重启达到阈值时应立即保留旧 upstream、输出有限诊断日志并终止，不得盲等完整健康超时。
7. **Maven 零或单生命周期** - release gate 所需目标测试、编译和最终 JAR 必须在同一干净、已审查源码 worktree 内通过 `./scripts/deploy/release-jar-manifest.sh build --tests '<tests>'` 完成；该入口仅在上述四项复用条件全部通过时运行零次 Maven 并沿用原可信 manifest/JAR，否则只执行一条 `mvn clean package -Dtest=<tests>`，成功后才写可信 manifest。squash merge 后必须改在 clean exact `origin/main` release worktree 校验 build commit 与前后 backend tree，再决定复用或单次安全回退。禁止用未受 manifest 约束的旧 `target/`、mtime 或先前分开的测试命令证明最终 JAR 已测试；protobuf 的 staleness 检查仍是实际构建路径的必要条件，除非有等价性能回归证据不得移除。
...
11. **统一发布总入口** - 预期合并后立即部署时，协调者应在最终审查与目标测试之后、合并之前调用 `./scripts/deploy/release-cretas.sh --phase build --base-sha '<dispatch Base SHA>' --tests '<tests>' --stage-backend YES-STAGE`，把唯一 Java/Web 构建和 Java 不可变制品上传前移；这不是部署，不安装、不重启、不切流。ACTIVE 归档和 scope 释放也应进入同一个最终 PR commit，禁止合并后为收尾再制造新 commit。合并后**先跑** `./scripts/deploy/prewarm-main-artifact.sh --tests '<同一选择器>' --wait 420`（幂等，可丢后台），看到 `PREWARM=done`/`already-warm` 再发布——`--prefer-ci-artifact` 虽已默认开，但合并后立刻发布时 CI 尚未产出制品，探测必然落空并回退本地构建（实测构建段 25s vs 204s），且 200s+ 的构建期间 `origin/main` 常被并发 session 推进，导致构建成功却因 exact-main 复检而整体作废（2026-07-30/07-31 各实测一次）。随后只从 clean exact `origin/main` 调用 `--phase deploy --confirm-prod YES-PROD`，由统一入口自动检测范围、复用 manifest/tree cache、部署/no-op 并输出 JSON 回执。它必须在任何昂贵 fallback 前短暂重取 `origin/main`，并在子部署启动前再次确认 exact main；开始生产发布后，本仓库协调者不得再合入无关 PR，直到统一回执完成。最终 `no-op`/`deployed` 必须取自 Java/Web 子部署回执，不能仅按 Git diff 推断；fallback build 必须反映在 `build_mode`、耗时和 build count 中。回执出现 `components.java.build="reused"`、`timings_seconds.java_build=0`、`build_count=0` 是正常成功，表示本次 deploy 调用复用了可信 manifest 且没有运行 Maven；build 子命令的缓存复用会强制目标选择器完全一致，但当前 deploy-phase manifest validate 只保留并校验记录的 selector 非空，并不把它与本次 `--tests` 参数比较，故宣称“本次 selector 已重新门禁”前必须读取 manifest 的 `target_tests`。默认安全串行；只有显式传入 `--parallel-if-independent YES-INDEPENDENT-SERVICES` 且全部风险检测未命中时才可并行。`--order backend-first|web-first` 只用于显式串行顺序。底层门禁不得复制或削弱。
12. **并行发布边界** - Java/Web 制品可在同一干净候选 worktree 用 `./scripts/deploy/release-cretas-artifacts.sh --tests '<tests>'` 并行构建；在 fork 两侧构建前，`release-java-preflight.sh` 必须先解析可执行 JDK、要求 Java 主版本至少 21，并验证显式测试选择器与项目导入。Java 侧复用命中时运行零次 Maven，否则只执行一次最终 `mvn clean package -Dtest=<tests>`；任一侧失败后必须立即取消另一侧进程组，使 Maven/Vite 孙进程也停止，禁止恢复为 `wait java; wait web` 的串行死等。生产并行部署仅限两边在任意切换顺序均 API 兼容的独立发布，且必须从 clean exact `origin/main` 使用 `deploy-cretas-parallel.sh --confirm-prod YES-PROD --confirm-independent-services YES-INDEPENDENT-SERVICES`。任何跨端接口、迁移、认证契约或需要严格先后顺序的改动必须串行发布。预检与复用门禁不能替代实际构建路径中的 Maven/Mockito 运行时验证。
...
15. **部署互斥判读** - `deploy-backend.sh`、`deploy-web-admin.sh` 与 `release-cretas.sh` 分别持有 `cretas-backend-deploy`、`cretas-web-admin-deploy`、`cretas-release` 三把不同的本机锁，外层编排与子部署不会自锁。遇到“另一个 deploy 进程持有锁”时禁止反射性 `rm`：`acquire_deploy_lock` 已会自动删除记录 PID 不存活的 stale lock；先读取 PID，以 `kill -0`/`ps`（Windows 可用 `Get-Process`）确认持有者及 Maven/Vite/部署孙进程均已退出，只有确认没有活跃发布后才可人工清理。
