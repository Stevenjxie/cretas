...
(function(){
  ...
  document.querySelectorAll('.v6-hero-mask img').forEach(function(img){
    gsap.fromTo(img, { scale: 1.35, yPercent: -12 }, { scale: 1, yPercent: 16, ease: 'none',
  ...
  if (stage && DESKTOP) {
    ...
    var CH = [
      { t:'① 认得出工序', d:'画面里正在做哪道工序，AI 自己判断 — 装盘、分割、包装，11 类工序无需人工登记，产线节奏第一次自动留痕。',
        ...
        p1:{left:'36%',top:'5%',width:'26%',height:'58%'}, p2:{left:'8%',top:'62%',width:'28%',height:'30%'} },
      { t:'② 数得清动作', d:'每一次装盘、每一件出品，AI 从画面里数出来当产量。谁在干活谁在空，节拍多少秒一件，车间产能看得见。',
        ...
        p1:{left:'6%',top:'56%',width:'48%',height:'38%'}, p2:{left:'22%',top:'8%',width:'18%',height:'44%'} },
      { t:'③ 盯得住合规', d:'帽子、口罩、手套没戴齐，AI 当场识别当场记录；画面异物实时报警 — 食品安全的红线，不靠人盯。',
        m:['穿戴合规检测','异物检测','当场记录'], a:'未戴手套 · 已记录', b:'合规复查已推送', warn:true,
        p1:{left:'34%',top:'36%',width:'22%',height:'26%'}, p2:{left:'56%',top:'44%',width:'20%',height:'28%'} }
    ];
    var cur = -1;
    function chapter(i){
      if (i === cur || !cap) return;
  ...
  document.querySelectorAll('.crow').forEach(function(row, i){
    ...
    gsap.from(row, { x: DESKTOP ? (i % 2 ? 320 : -320) : 0, y: DESKTOP ? 0 : 80, opacity: 0, duration: 1.25, ease: 'expo.out',
