 ...
 * **全站没有界面** —— 用户被告知去删, 却没有地方能删。
 *
 * ⚠️ 2026-08-13 订正: 当初补的出口是「逐行 DELETE /bom-recipes/items/{id}」,
 * **那条路结构上不可能成功**(见 BomRecipeServiceImpl#reconcileUpgradedInputSkeletons
 * 的说明: 挑选条件与 deleteItem 的拒绝条件完全相同)。现在改成把「人已确认」回传给
 * ensure-draft, 由后端删它自己算出的孤儿行。**本函数只负责"有没有孤儿、是哪几行(给人看)",
 * 不再负责"怎么删"。**
 *
 * 这个函数是那个出口的判据: 挑出「绑着画布槽位、而该槽位已不在当前画布上」的配方行。
 *
