  ...
  const doSubmit = async (qty: number, parsed: ParsedReportFields) => {
    ...
    try {
      const commonPayload = {
        ...
        warehouseOutQuantity: parsed.warehouseOutQuantity,
        feedInQuantity: parsed.feedInQuantity,
        carryoverQuantity: parsed.carryoverQuantity,
          ...
          <Card style={styles.card}>
            <Card.Content>
              ...
              <View style={styles.twoCol}>
                <TextInput
                  testID="report-warehouse-out-input"
                  label="领料出库"
              ...
              <TextInput
                testID="report-carryover-input"
                label={`本批剩余/结转 (${unit || 'kg'}, 不填则自动算)`}
                value={carryoverQty}
