...
const ROLE_CARDS: Record<string, (props: QuickActionCardGridProps) => CardConfig[]> = {
  ...
  warehouse_manager: ({ onSendIntent, onNavigate }) => [
    ...
    // 仓储树里正规的入口是「待收货」列表 WHPurchaseReceiveList(无参)。
    // ⚠️ 不能用 WHUnorderedInboundReceive: 它必须带 noticeId, 不是空白新建。
    { icon: 'package-down', label: '原料入库', description: '扫码或手动入库', color: '#2196F3', action: () => onNavigate('WHPurchaseReceiveList') },
