...
PR：[#2803](https://github.com/Stevenjxie/cretas/pull/2803)（5 个 commit，全部带闸 + 变异对照）

---

## 0. 一句话结论

**判据八（忘了调拨）在 web 端已经做对了**，且本轮把它旁边那句会误导人的话修好了；
**判据七（仓储/调拨）走通并逐个数字对上**；
**判据二（两端实时通）没通过** —— App 报工与 web 逐道录入**不是同一条账**，
...
- 判据三：半成品**确认**不在可售列表 ✅ / 原料可开单 ✅（现有单据里「鸡油」就是原料行）；
  **辅材、包材未单独开单验证**；「都必须是入过库的东西」这条**未验** ——
  可售来源是 `raw-material-types/active`（全部启用的原料类型，**无入库过滤**），
  是否在下单或发货处拦住未验。
- 判据四：web 端「无订单入库」**未验**。
