---
name: ux-flow
description: 低技术素养用户屏幕的 UX Flow Gate。Use before proposing approaches for React Native screens involving operators, warehouse workers, quality inspectors, processing, warehouse, quality-inspector paths, or workflows such as 报工、入库、出库、盘点、质检、扫码收货、发货.
---
...
1. 读取 `.codex/rules/fool-proof-design.md`，对照 5 条规则逐一检查
...
开发者编辑 P1 路径下的 `.tsx` 文件时，`.codex/hooks/ux-advisory.js` 自动运行，输出以下检查清单（advisory，不阻断）：
