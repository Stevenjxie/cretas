...
public class RawMaterialType extends BaseEntity {
    ...
    /**
     * Optional L3 taxonomy node ID used only for classification and filtering.
     * It is deliberately independent from {@link #code}.
     */
    @Column(name = "classification_segment_id")
    private Long classificationSegmentId;

    /** The single user-facing material code. */
    @Transient
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "tax_treatment", nullable = false, length = 16)
    private TaxTreatment taxTreatment = TaxTreatment.TAXABLE;

    @Column(name = "tax_exemption_reason", length = 255)
    ...
    /**
     * Optional L1 taxonomy code, such as 001=原料/002=包材/003=辅料.
     */
