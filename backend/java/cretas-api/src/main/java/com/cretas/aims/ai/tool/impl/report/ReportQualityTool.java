 ...
 * Quality Report Tool
 *
 * Provides quality-related reports including inspection results,
 * quality pass rates, defect analysis, and quality trends.
 *
 * @author Cretas Team
 * @version 1.0.0
 * @since 2026-01-07
...
@Slf4j
@Component
public class ReportQualityTool extends AbstractBusinessTool {
    ...
    public String getDescription() {
        return "获取质量报表，包含检验结果统计、质量合格率、缺陷分析、质量趋势等数据。" +
                "适用场景：质量分析、合格率统计、缺陷追踪、质量改进跟踪。";
    }
    ...
    public Map<String, Object> getParametersSchema() {
        ...
        Map<String, Object> inspectionType = new HashMap<>();
        inspectionType.put("type", "string");
        inspectionType.put("description", "检验类型筛选");
        inspectionType.put("enum", Arrays.asList("INCOMING", "IN_PROCESS", "FINAL", "ALL"));
        properties.put("inspectionType", inspectionType);
    ...
    protected Map<String, Object> doExecute(String factoryId, Map<String, Object> params, Map<String, Object> context) throws Exception {
        log.info("执行质量报表查询 - 工厂ID: {}, 参数: {}", factoryId, params);
        ...
        String inspectionType = getString(params, "inspectionType");
        ...
        // 调用服务获取质量报表数据
        Map<String, Object> qualityData = reportService.getQualityReport(factoryId, startDate, endDate);

        // 构建返回结果
        Map<String, Object> result = new HashMap<>();
        result.put("reportType", "QUALITY");
        ...
        if (inspectionType != null) queryConditions.put("inspectionType", inspectionType);
        result.put("queryConditions", queryConditions);

        log.info("质量报表查询完成 - 工厂ID: {}, 日期范围: {} ~ {}", factoryId, startDate, endDate);
    ...
    protected String getParameterQuestion(String paramName) {
        Map<String, String> questions = Map.of(
            "period", "请问您想查看哪个时间段的质量数据？可选：今日、本周、本月、本季度、本年。",
            "startDate", "请问报表的开始日期是？（格式：yyyy-MM-dd）",
            "endDate", "请问报表的结束日期是？（格式：yyyy-MM-dd）",
            "inspectionType", "请问要查看哪种检验类型？可选：来料检验、过程检验、最终检验、全部。"
        );
