 ...
 * 后才 409 拒绝不属于生产计划 BOM 的原料批次。本只读端点把<b>同一份判定逻辑</b>
...
public class ProductionSettlementBomEligibilityResponse {
    ...
    /**
     * restricted=true 时，该计划是否找到固化 BOM（旧计划无 pin 时回退当前生效 BOM）。
     * <p>false = 没有可用 BOM（镜像后端 409 "该生产计划没有可用 BOM，不能直接核对原料领用"），
     ...
     * （镜像后端 409 "该生产计划 BOM 没有原料明细，不能直接核对原料领用"）。
     */
    @Builder.Default
