...
public class ProductionPlan extends BaseEntity {
    ...
    /** Customer identity frozen from the authoritative sales order. */
    @Column(name = "customer_id", length = 191)
    private String customerId;

    /** Sales processing-mode snapshot; null is retained only for legacy orders. */
    @Enumerated(EnumType.STRING)
    @Column(name = "processing_mode", length = 32)
    private SalesProcessingMode processingMode;

    /** Material-supply-mode snapshot; null is retained only for legacy orders. */
    @Enumerated(EnumType.STRING)
    @Column(name = "material_supply_mode", length = 32)
    private MaterialSupplyMode materialSupplyMode;

    /** Legal ownership of output inventory produced by this pinned plan. */
    @Enumerated(EnumType.STRING)
