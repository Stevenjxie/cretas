...
VALUES (
    ...
    NULL, 'MEDIUM',
    '["留样","食品留样","48小时留样","留样追踪","留样管理","留样巡检","今天哪些留样要销毁","留样到期","留样过期","GB 31654","125克","冷藏留样","留样还在吗","查留样"]'::jsonb,
...
VALUES
    (gen_random_uuid(), 'FOOD_SAMPLE_CREATE', '创建食品留样', 'DATA_OP',
     ...
     '["留样","创建留样","新建留样","留样这批","今天这批留样","卤猪蹄留样","留样 130g","记录留样","存留样"]'::jsonb,
    ...
    (gen_random_uuid(), 'FOOD_SAMPLE_QUERY_PENDING_DISPOSE', '查待销毁留样', 'QUERY',
     ...
     '["待销毁留样","过期留样","48小时到了","48h 到了的留样","今天哪些留样要销毁","留样过期列表","巡检留样","逾期留样","到期留样"]'::jsonb,
    ...
    (gen_random_uuid(), 'FOOD_SAMPLE_DISPOSE', '销毁/送检留样', 'DATA_OP',
     ...
     '["销毁留样","留样销毁","标记销毁","已销毁","留样送检","拿去送检","客户投诉送检","用于检测","送疾控检测","INSPECTION","CONTAMINATED"]'::jsonb,
    ...
    (gen_random_uuid(), 'FOOD_SAMPLE_TRACE', '按批次追溯留样', 'QUERY',
     ...
     '["留样追溯","B- 留样还在吗","查 B- 的留样","客户投诉找留样","批次留样","留样查询","这批留样"]'::jsonb,
