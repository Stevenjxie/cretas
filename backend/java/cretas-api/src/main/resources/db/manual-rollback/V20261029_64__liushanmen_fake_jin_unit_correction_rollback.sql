-- =============================================================================
-- 回滚 V20261029_64 —— 把六膳门的假换算与「斤」标签原样放回去
--
-- 手动执行, Flyway 不跑本目录。
--
...
DECLARE
    ...
    v_pri  integer := 0;
BEGIN
    IF to_regclass('public.migration_jin_unit_fix_20261029_64') IS NULL THEN
        RAISE NOTICE '台账表不存在, V20261029_64 未执行过, 无需回滚';
        RETURN;
    END IF;
    ...
    UPDATE material_packaging_specs s
      ...
      FROM migration_jin_unit_fix_20261029_64 l
    ...
    RAISE NOTICE 'V20261029_64 回滚: 包装规格 % 条 / 采购单行 % 条 / 收货行 % 条', v_spec, v_poi, v_pri;
END $$;

-- 台账刻意保留; 要彻底清掉再手动:
--   DROP TABLE migration_jin_unit_fix_20261029_64;
