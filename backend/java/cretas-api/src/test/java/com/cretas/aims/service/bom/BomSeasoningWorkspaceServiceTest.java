...
class BomSeasoningWorkspaceServiceTest {
    ...
    void workspaceKeepsEmptyProcessesAndDeduplicatesMaterialWithoutSummingDosage() {
        ...
        when(productWorkProcessRepository.findByFactoryIdAndProductTypeIdOrderByProcessOrderAsc(
                FACTORY, "product-1")).thenReturn(List.of(process("p1", 1), process("p2", 2), process("p3", 3)));
        ...
        BomSeasoningItem first = binding(1L, "p1", "salt", "5.0000");
        BomSeasoningItem second = binding(2L, "p2", "salt", "1.5000");
    ...
    void createAllowsSameMaterialInAnotherProcessAndUsesAuthoritativeSnapshot() {
        ...
        when(recipeRepository.findById(RECIPE)).thenReturn(Optional.of(recipe));
        when(productWorkProcessRepository.existsByFactoryIdAndProductTypeIdAndWorkProcessId(
                FACTORY, "product-1", "p2")).thenReturn(true);
        RawMaterialType material = material("salt");
        when(materialTypeRepository.findById("salt")).thenReturn(Optional.of(material));
        when(seasoningItemRepository.findByRecipeIdAndWorkProcessIdAndMaterialTypeId(
                RECIPE, "p2", "salt")).thenReturn(Optional.empty());
        when(seasoningItemRepository.findByRecipeIdAndWorkProcessIdOrderBySeqAsc(RECIPE, "p2"))
                .thenReturn(List.of());
        ...
        SeasoningBindingCreateRequest request = createRequest(4L);
    ...
    void duplicateInSameProcessReturnsConflictWithExistingBinding() {
        ...
        when(recipeRepository.findById(RECIPE)).thenReturn(Optional.of(recipe));
        when(productWorkProcessRepository.existsByFactoryIdAndProductTypeIdAndWorkProcessId(
                FACTORY, "product-1", "p1")).thenReturn(true);
        when(materialTypeRepository.findById("salt")).thenReturn(Optional.of(material("salt")));
        when(seasoningItemRepository.findByRecipeIdAndWorkProcessIdAndMaterialTypeId(
                RECIPE, "p1", "salt")).thenReturn(Optional.of(binding(9L, "p1", "salt", "5")));

        BusinessException ex = assertThrows(BusinessException.class,
                () -> service.createBinding(FACTORY, RECIPE, "p1", createRequest(4L)));
    ...
    void staleRevisionReturnsReloadConflictAndDoesNotWrite() {
        ...
        when(materialTypeRepository.findById("salt")).thenReturn(Optional.of(material("salt")));
        when(seasoningItemRepository.findByRecipeIdAndWorkProcessIdAndMaterialTypeId(
                RECIPE, "p1", "salt")).thenReturn(Optional.empty());
        when(recipeRepository.claimSeasoningRevision(RECIPE, FACTORY, 3L)).thenReturn(0);

        BusinessException ex = assertThrows(BusinessException.class,
                () -> service.createBinding(FACTORY, RECIPE, "p1", createRequest(3L)));
    ...
    private BomRecipe recipe(BomRecipe.Status status, long revision) {
        return BomRecipe.builder().id(RECIPE).factoryId(FACTORY).recipeCode("BOM-1")
                ...
                .status(status).seasoningRevision(revision).build();
    }
    ...
    private BomSeasoningItem binding(Long id, String process, String material, String dosage) {
    ...
    private SeasoningBindingCreateRequest createRequest(long revision) {
