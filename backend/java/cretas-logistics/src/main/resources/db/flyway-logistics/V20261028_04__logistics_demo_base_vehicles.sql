...
DO $$
BEGIN
    -- Fresh Flyway databases do not have the Hibernate-owned vehicles table yet.
    -- Existing/prod databases retain the original idempotent demo seed behavior.
    IF to_regclass('public.vehicles') IS NULL THEN
        RAISE NOTICE 'vehicles does not exist yet; skipping DEMO_LOGISTICS vehicle seed';
        RETURN;
    END IF;
