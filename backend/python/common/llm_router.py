...
#: 绝对置底。目前**空** —— 见 `_ABSOLUTE_FIRST` 里 2026-08-16 的实测。
#: ⛔ 机制保留不删: 下次再出现「某个候选会稳定超时」时, 结构键要现成的。
_ABSOLUTE_LAST: frozenset = frozenset()

#: 绝对置顶 —— owner 2026-08-16:「用 zhipu 的优先, 其他的不用动」。
#:
#: 🔴 这一条**推翻**了 08-09 事故留下的「zhipu 钉最后」。推翻的依据是当天实测,
#:    不是转述: 用生产路由自己的 `_apply_slot_params` 构造 payload, 对
#:    CHAT/MAPPER/REASONING 三个槽各跑 8 轮, 并**同时量它前面那两个健康候选**
#:    做跨样本对照(只量 zhipu 的话, 分不清是它慢还是这条网络慢):
#:
#:      槽          zhipu 中位/P95      aistore 中位/P95    deepseek 中位/P95
#:      CHAT        0.53 / 0.67         1.51 / 1.58         1.43 / 1.60
#:      MAPPER      0.77 / 2.11         1.70 / 1.96         1.29 / 2.07
#:      REASONING   0.64 / 2.14         1.49 / 2.63         1.35 / 1.48
#:
#:    两轮共 24 次调用, zhipu **24/24 成功、零超时、零空 content**,
#:    而且中位时延是三家里最快的。08-09 那句「它在某些槽上会稳定超时」
#:    在今天**量不出来**。
#:
#: ⚠️ 如实记两条没那么好看的:
#:    · zhipu 的尾部相对自己的中位更重(MAPPER 0.77→2.11 = 2.7×,
#:      aistore 只有 1.15×)。它不是"哪一项都最快", 是"典型最快、最坏持平"。
#:    · 探针在本机跑单跳, **量不到生产负载下整链共享预算的行为** ——
#:      也就是 08-09 事故真正的那个机制。
#:
#: 🔑 所以敢改, 靠的不只是这次读数, 还有一个结构事实:
#:    那个"慢的第一跳吃光预算"的机制**本身已经被修掉了** ——
#:    `_reserve_for_fallback` 会在快槽上给后面的候选预留一段预算。
#:    排序上的钉死是叠在那道结构修复之上的第二道保险, 不是唯一的保险。
_ABSOLUTE_FIRST: frozenset = frozenset({("zhipu", "glm-4.5-air")})
...
def _build_chain(slot: SLOT) -> List[Tuple[str, str]]:
    ...
    return _dedup_chain(sorted(entries, key=lambda p: (
        ...
        #          一张会变的数据表里涌现出来。**
        # ⛔ 置顶必须是**第一位**排序键 —— 排在「地板置底」后面的话,
        #    zhipu 属于 _TEXT_TAIL, 那一位会先把它压到底, 置顶永远不起作用。
        #    (这正是 08-13 记的那个教训的反面: 一条位置不变量只能由结构键保证,
        #     而结构键的**次序**本身就是那个保证的一部分。)
        0 if p in _ABSOLUTE_FIRST else 1,
