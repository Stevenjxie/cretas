...
5. 当前仍有少数 T3 问法没有稳定落到可执行的正确 resolver，即使用户已经写了时间也会错误澄清或报执行维度不匹配；已知包括“最近30天哪个时段生意最好”（时段问句路由已修，但仍缺历史时段表现 resolver）、“最近30天食材成本占营收多少”“最近30天折扣力度多大”。T3 还有轮次波动，这是当前未闭环限制，不能宣称所有条件齐全的问法都已稳定回答。

例如首问“最近30天总营收是多少”或“最近30天加权毛利率是多少”而未写门店时，当前可回答全部门店合计并披露范围；上述时段、食材成本占比和折扣问法仍未闭环。首问“比较 A 店宫保鸡丁 4–6 月销量”，追问“那成本和毛利呢”可沿用三项范围；若改问“换 B 店看 7 月”，则按新范围重新规划。"""
...
def _needs_restaurant_department_stocktake_guard(query: str) -> bool:
    ...
    mentions_ai_workspace = any(
        term in normalized for term in ("ai 工作台", "ai工作台", "ai 价值汇总", "ai价值汇总", "ai-value")
    )
...
3d. 【餐饮 SmartBI 会话范围】导览助手只能解释以下合同，不得假装已经替用户运行分析：首轮新问题没有点名门店，且 planner 结构化判定唯一缺项是 `store_scope` 时，默认查询全部门店并在答案显式披露“范围：全部门店合计”；只有取得真实门店目录时才可披露家数。当前“最近30天总营收是多少”“最近30天加权毛利率是多少”等问法已生效；但“最近30天哪个时段生意最好”（时段问句路由已修，仍缺历史时段表现 resolver）、“最近30天食材成本占营收多少”“最近30天折扣力度多大”仍可能错误澄清或报执行维度不匹配，且 T3 有轮次波动，属于未闭环限制，禁止宣称所有条件齐全问法都稳定回答。用户明确写出的有效门店/全部门店优先，同一连续会话的兼容门店、菜品和时间可沿用，最新条件覆盖旧条件。“全部门店”只能是聚合范围。点名未知/停用门店、正在回答上一轮澄清的延续轮、缺时间或其它必需条件时仍须澄清，不能用默认全店绕过；先得全店答案后再说某店属于新问题并重新规划。清空对话会重置会话；跨页面筛选不保证自动带入，禁止宣称“跨模块联动锁定”。
...
3m. 【五部门与金额权限】运营、市场、财务、人事、采购五个部门按各自事实源与权限展示；人事缺在岗人数和目标人效时显示缺项空态，财务缺成本覆盖时显示“—”，都不得造假 KPI。餐饮老板为全局读写角色；店长管运营并只读人事，市场、财务、人事、采购分别由对应部门角色管理，厨师长已退役。左侧顶层“AI 工作台”菜单进入 `/dashboard/ai-value` 查看 AI 价值汇总，当前菜单与路由均已接通；角色可见、可管理、可写和金额读取仍是不同权限，菜单、路由和金额请求都必须经过后端中央角色/模块/金额闸，不能靠前端隐藏代替授权。
