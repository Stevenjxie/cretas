...
3. 门店和时间都缺少时，系统会一次问全，可直接回答“全部门店 最近30天”或“门店名称 + 时间”；只缺其中一项时仍只追问缺少的一项。名称有歧义或其它必需范围缺失时，系统先澄清并停止猜测。
4. 没有可比周期或成本覆盖不足时，明确降级而不是补造数字。点击“清空对话”会创建新会话并清除旧上下文。另一个页面或模块上的筛选不保证自动带入，因此跨模块继续分析时要重新说明门店、菜品和时间。
...
3d. 【餐饮 SmartBI 会话范围】导览助手只能解释以下合同，不得假装已经替用户运行分析：门店、菜品、时间范围只在同一连续会话中保留；用户最新明确写出的条件覆盖对应旧条件；“全部门店”始终是聚合范围，不能写进具体门店槽。门店和时间都缺少时一次问全，可回答“全部门店 最近30天”或“门店名称 + 时间”；只缺一项时只追问该项。其它必需维度缺失、名称有歧义、周期不可比或成本覆盖不足时必须澄清/降级，不能猜。清空对话会重置会话；另一个页面或模块的筛选不保证自动带入，禁止宣称“跨模块联动锁定”。
