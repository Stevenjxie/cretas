...
普通质检照片证据与包装标签对象复核是两条独立链，不能用“提交成功”或 AI 初筛代替照片回读与人工真值。

**普通质检照片：**
1. 手机拍照后把照片带回质检表单；质检记录成功创建并取得记录 ID 后，逐张登记为 `QUALITY_CHECK` 附件。记录创建成功与每张照片上传成功要分别报告，单张上传失败会保留已创建质检记录并明确告警，不能冒充证据齐全。
2. 质检详情从附件子系统回读照片，也允许在有权限时补传。验收必须看到正确质检记录 ID、附件数量和可打开的原图；空的旧 `photos` 字段不能作为“没有照片”的依据。

**包装标签对象复核：**
1. 进入标签待审任务，逐张查看原图及盒子、白标、彩标三层参考框和 AI 提议；所有照片都必须进入人工审核，AI 正确框默认保留，但只有人工完成整图复核后才成为认可框。AI 为 0 候选也必须人工检查，不能直接判合格。
...
**仓库手机端：**
1. 旧“AI 智能入库”、空白“新建入库”和生产看板“原料入库”都曾调用已停用的无来源建批次接口，现已退役。仓库必须从“仓储待收货 / 采购收货列表”选择真实采购单、来货通知或其它已登记来源进入正规收货；生产角色没有可用的仓储收货入口时只显示去处提示，不能保留一个提交必失败的按钮。
2. 收货列表进入上架页后，选择库位并确认会实际更新该批次的 `storageLocation`；只有接口成功后才显示“上架成功”。验收要独立刷新/回读新库位，不能用 Toast 证明已经上架；失败时停留当前页并显示真实原因。
3. “库存记录”显示的是当前已加载页面，不冒充“今日/全部”，顶部总数是批次数，不是跨单位数量合计。每个批次使用实际库存单位；只有同为 kg 的行才汇总 kg 小计，箱、袋、瓶等不与 kg 相加。换算只读取该物料自己的包装层级，缺少规格明确显示“未设规格”，绝不按 1:1 猜测。
...
4. **门店×时段排序必须同源：** 用户要求按门店拆时段表现时，先找业务量最强的首位时段；有金额权限按该时段营收、无金额权限按该时段单量排序。首段点名的最强门店必须就是表格第一行，并显式写“按该时段营收/单量从高到低”，不能让首段按单时段、表格却按各时段总计排序。
5. **结论要能做决定：** 门店毛利或菜品毛利回答先明确“哪些店/菜在亏或没有可比成本”，再给基于当前差距的动作；门店差异很小时不机械要求逐店排查。数字来自确定性查询，导览助手只解释入口和方法，不代算真实门店数据。
...
餐饮日结的时间窗、输出偏好、菜品表和追问按钮使用同一份已封存查询上下文；呈现层只排版已有数字，不另算第二套口径。

1. **时间窗：**“本季度 / 上个季度”由代码按自然季度计算；进行中的本季度右端点最多到今天。用户说了系统尚不能直接识别的时间词而采用近似窗口时，正文必须披露实际使用的起止范围；绝对区间晚于今天时截到今天。由答案生成的追问按钮把当前屏幕的时间窗直接写进问句，例如最近 30 天的“按日期看毛利”仍查询最近 30 天，不能被更早历史对话改成本月。
...
4. 日结菜品摘要已经消费表格偏好并输出 GFM 三项金额表；其它问答、图表或报告文件仍只有在当前客户端接入对应渲染/下载动作并实际返回结果后才能宣称已经生成。没有实际渲染或下载结果时，不能宣称“表格/报告文件已生成”。
...
def _needs_label_qc_review_guard(query: str) -> bool:
    """Explain persisted QC evidence and isolated label-review truth layers."""
    normalized = (query or "").lower()
    mentions_label = any(
        term in normalized
        for term in (
            "标签", "白标", "彩标", "质检照片", "照片证据", "quality_check",
            "单盒", "托盘", "裁切",
        )
    )
    mentions_review = any(
...
def _needs_factory_rn_readonly_guard(query: str) -> bool:
    ...
    rn_signals = sum(
        term in normalized
        for term in (
            "盘点记录", "物料需求单", "智能入库", "仓库角色", "电脑端",
            "待收货", "采购收货", "新建入库", "原料入库", "上架",
        )
    )
...
def _needs_restaurant_daily_presentation_guard(query: str) -> bool:
    """Keep daily tables, time windows and generated follow-ups on one contract."""
    normalized = (query or "").lower()
    daily_table = any(
        term in normalized
        for term in (
            "日结表格", "打烊表格", "菜品表", "三列表", "成本比营收",
            "成本高于营收", "缺成本卡为什么", "毛利率列",
        )
    )
    output_negation = any(
        term in normalized
        for term in (
            "别用表格", "不要表格", "只用文字", "文字就行", "别用图",
        )
    )
    time_window = any(
        term in normalized
        for term in (
            "上个季度", "本季度", "静默换窗口", "窗口替换", "截到今天",
        )
    )
    button_context = "追问按钮" in normalized and any(
...
3i. 【餐饮日期、繁体范围与输出偏好】明确起止日期按自然日闭区间并标记“指定区间”，无效或倒置日期必须澄清；本季度/上个季度由代码按自然季度计算，进行中窗口和未来绝对日期不得越过今天；模型近似替换用户时间词时必须披露实际窗口。由系统生成的追问按钮把当前答案时间窗写进问句，不能从更早历史继承另一窗口。“全部門店”只作为与“全部门店”等价的聚合范围，当前繁体支持不得扩大成全句转换。用户明确要求文字/表格/图表/报告文件时优先，未明确时当前默认文字+表格，文字始终保留；“别用表格/只用文字”等否定必须真的移除表格，不能反向锁死。输出偏好不改变查询计划。排行、对比、构成和预警类真实结果统一使用 GFM Markdown 表格，必须保留表格前后空行，数字右对齐，缺失显示“—”而不是 0；没有金额权限时省略金额列。日结菜品表已消费该偏好，固定展示菜品/营收/成本/毛利，不显示单菜毛利率；缺卡明确写“缺成本卡”，成本高于营收的菜要点名并先指向核成本卡。门店简称匹配多家时返回最多 3 个真实候选按钮，简称或片段唯一匹配时也先给真实门店确认按钮，只有用户完整写出有效全名才直接使用；范围收窄必须保留原问题指标、菜品和时间，客户文案不得暴露“问题对象/分析范围/语义规划/查询计划/计划版本”等内部概念。澄清轮保持原问题已封存的指标，只有 resolver 真支持时间窗时才显示时间按钮。只有呈现层实际返回对应表格、图表或下载文件时才能宣称已生成，导览助手不得伪造输出。
...
3q. 【打烊经营摘要】“今天怎么样”按当天、当前门店汇总营收、订单与毛利；任一部分为估算则整体标为估算。表格偏好生效时，菜品表只排版结构化毛利事实，披露成本卡覆盖、截断差额、理论依据以及套餐/单品尚未拆分；缺卡不填 0。成本高于营收的有卡菜最多点名 3 道并建议先核卡，不能直接据此调价或下架。无业务和无数据分开且均不主动推送，金额权限继续生效。它不是逐食材理论消耗与盘存偏差的日清日结，不能冒充食材日清。
...
- 普通质检照片在质检记录成功创建后逐张登记为 `QUALITY_CHECK` 附件；记录成功与附件上传成功分别报告，详情必须回读可打开原图，单张失败不能冒充证据齐全。
- 包装标签拍检是另一条独立流程：AI 候选无论 0 处还是多处都进入人工审核；复核台按盒子、白标、彩标三层参考框显示。切换盒子/照片即校验保存，矛盾状态阻止切换；只按工厂最终认可盒子生成幂等单盒裁切，平台精修不覆盖工厂真值。人工审核不等于报工工时，训练批准只允许导出，不会自动训练或发布模型。
- 报工页面按配置单位显示和提交。kg/g 等同量纲可科学换算；盒、袋、只等计数/包装单位按字面量匹配且不得暗中折重。跨量纲成品率必须先补充每单位重量，否则明确不可比，不能输出伪造百分比。
- RN App 的业务入口只面向仓库主管和仓库操作员；其它 10 个业务角色使用电脑 Web，平台管理员和超级管理员仅有技术支持例外。旧 AI/空白新建入库与生产侧原料入库已退役，仓库从待收货/采购收货列表选择真实来源；上架只有实际更新并回读 `storageLocation` 后才成功。RN 库存记录总数是批次数，只汇总同为 kg 的行，包装换算只读当前物料层级，缺少规格显示“未设规格”。
