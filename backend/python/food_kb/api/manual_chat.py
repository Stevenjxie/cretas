...
**同一会话的范围规则：**
1. 首轮新问题没有点名门店、但时间和其它执行条件已经足够时，系统默认查询全部门店，不再把“选择门店”当成必答题；答案必须显式说明“范围：全部门店合计”，取得真实门店目录时才可写具体门店数，不能猜数量。
...
例如首问“最近30天总营收是多少”而未写门店时，直接回答全部门店合计并披露范围；首问“比较 A 店宫保鸡丁 4–6 月销量”，追问“那成本和毛利呢”可沿用三项范围；若改问“换 B 店看 7 月”，则按新范围重新规划。"""
...
3d. 【餐饮 SmartBI 会话范围】导览助手只能解释以下合同，不得假装已经替用户运行分析：首轮新问题没有点名门店、但时间和其它执行条件齐全时，默认查询全部门店并在答案显式披露“范围：全部门店合计”；只有取得真实门店目录时才可披露家数。用户明确写出的有效门店/全部门店优先，同一连续会话的兼容门店、菜品和时间可沿用，最新条件覆盖旧条件。“全部门店”只能是聚合范围。点名未知/停用门店、正在回答上一轮澄清的延续轮、缺时间或其它必需条件时仍须澄清，不能用默认全店绕过；先得全店答案后再说某店属于新问题并重新规划。清空对话会重置会话；跨页面筛选不保证自动带入，禁止宣称“跨模块联动锁定”。
