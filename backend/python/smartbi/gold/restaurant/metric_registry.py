...
# ── 成本卡异常检测 ─────────────────────────────────────────────────────
#
# 🔴 owner 2026-08-13 裁定: **一个机制同时满足三件事** —— 排除的依据、闸的判据、
#    正文里那句话的内容。⛔ 不要写成三份。
#
# prod 实测(RES_3101_009 / 2026-08-12)催生了它:
#   米饭  成本卡 ¥167.20  售价 ¥16.80  成本率 1117%  → 一道菜占 covered_cost 的 87%
#   对照同租户「米饭(单人份)」成本卡 ¥2.20 —— 同一样东西两张卡差 76 倍, 单位错了。
#
# ⚠️ 这两类排除**性质不同, 不许混成一条规则**:
#   · 异常成本值   = 数据质量问题, **暂时的** —— 卡修好了要**自动回到**分子分母里
#   · 米饭/附属用品 = 业务口径判断, **长期的** —— 不需要自愈
#   前者靠「按当天数据现算」自愈: 判据是比值, 不是名单, 卡一改比值就变。

#: 成本 > 售价 → 可疑。计数并在正文里说, **不排除**(餐饮确实可能亏本卖)。
COST_SUSPECT_RATIO = 1.0
#: 成本 > 5×售价 → 基本确定是单位错(一袋当成一份)。排除, 并**指名道姓**。
#: ⛔ 阈值取 5 而不是 2: 亏本引流最多亏到成本的 2~3 倍售价, 5 倍以上不是定价策略。
