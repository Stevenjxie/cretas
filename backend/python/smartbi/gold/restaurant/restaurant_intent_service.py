...
def _dimension_gap_advice(spec: RestaurantQuerySpec,
    ...
    if both:
        ...
        return (
            ...
            f"这次选中的算法拆不出这一层。"
            f"想看的话分开问：先只按{_names(both)}问同一件事。"
        )
    ...
    # 🔴 2026-08-18: 这里原来给的是一句**可照抄的引号问句**
    #    （「换成问「按食材怎么样」我就能答。」）。
    #    📏 prod 实测把它原样问一遍: **4/4 兑现不了** —— 老板照抄之后
    #    又被反问一次「你想按食材看哪项指标？」。
    #
    # ▎一句兑现不了的承诺，比不给建议更糟：他照做、失败一次，之后就不再照做，
    # ▎而那时我们真正给对的建议也一起失效。
    #
    # ⛔ 修法**不是**把话题拼进去。逐条实测过(2 轮一致, 阳性对照 2/2):
    #      按食材看库存预警 / 盘点差异 / 食材损耗   2/2 ✅
    #      **最近30天**按门店看时段表现            2/2 ✅ (还要补时间窗)
    #      **全部门店 最近30天** 按门店看营收与订单  **0/2 🔴 (补全两个槽仍然不行)**
    #    ⇒ **没有一个构造规则能保证兑现**。按模板发 = 把这 4 条换成另外若干条。
    #
    # ⇒ 改成描述**动作**，不给可照抄的字面问句。老板照做时用的是**他自己的话**，
    #    话题因此天然在句子里 —— 📏「哪些食材最缺货」「按食材最缺货」都 2/2 ✅。
    #    他若组出来的话仍然含糊（原问句本来就缺话题，例如「趋势」），
    #    那一次反问是**正确的**：是他没说全，⛔ 不是我们给了句假话。
    return (
        f"你这句要按{_names(extra)}来看，而这次这个数我只能按{_names(supported)}"
        f"给你，按{_names(extra)}拿不到。"
        f"换成按{_names(supported)}问同一件事，我就能答。"
    )
        ...
        if not contract.passed or not displayable:
            ...
            safe_text = partial_text or (
                (
                    ...
                    # 🔴 2026-08-17 当天回归：第一版这里硬编码了
                    #    「例如把**为什么**换成…」，而这条文案会发给**任何**
                    #    产出被驳回的问句。实测「有没有菜是卖一份亏一份的」
                    #    拿到了它 —— 老板**根本没问「为什么」**。
                    # ▎那就是我正在修的那个毛病本身：**假建议**。
                    #   一句听起来很具体、其实与他的问题无关的指路，
                    #   烧掉的正是「敢说不准」那点本钱。
                    # ⇒ 换成**不假设问句形状**的说法。⛔ 不要在这里猜他问了什么
                    #   —— 手边没有可靠的问句分类, 而猜错的代价比说得笼统高。
                    f"你可以把问题问得更小一点（限定到某家店、某道菜或某个时间段），"
                    f"那条路我能给你确定的数。"
                )
