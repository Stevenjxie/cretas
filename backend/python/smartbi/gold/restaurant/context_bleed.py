...
⚠️ 「省略」在这里**不是**日常义的「问句没提到 X」。它是两件具体的事，
   而且**不是并列的备选** —— 中间隔着一道闸（实际判定顺序见 `explain()`）：

    闸 ·  自足的问句什么都不该继承 —— 问句里没有指代/省略词
          ⇒ 差异集合里的每一项都算串。
          ⛔ 「槽位本身为空」对自足问句**不成立**，到不了下面那一条。
    过闸后 · 槽位本身为空（不带 history 时压根没解析出来）
          ⇒ 问句确实没说，继承是补全。

  ⇒ 日常义的读法（「问句没提到门店 ⇒ 继承门店有解释」）会让这套判据在
    上面那段客户转录的原形上返回 0 条 —— 「这个月毛利多少」两个槽位皆空。

「指代/省略词」**以 `ANAPHORA` 元组为准**，⛔ 不以这段举例为准：
`呢 / 它 / 刚才 / 上面 / 同期 / 环比 / 同比 / 还有 / 再看 / 接着 / 继续 /
换成 / 换个 / 分别`。
⚠️ 「这个月」「那天」这类**不算**指代 —— 它们在 `NOT_ANAPHORA` 反向名单里，
   判定前先被剔掉（理由见该常量上方的注释）。

⚠️ 反过来：**问句自己明确说了的槽位，继承值不许覆盖它** —— 那是「覆盖式」串，
   比「凭空多出来」更隐蔽，因为两边都有值、只是值不同。
   它是**闸前的短路**（`explain()` 的第一个分支），比自足性判定还早。
"""
...
def explain(query: str, slot: str, before: Any, after: Any) -> Optional[str]:
    ...
    ⛔ 下面四条**不是并列的规则**，是**有顺序的控制流** —— 前两条是闸，
       只有过了闸才到得了「解释得通」。写成并列会被读成「任一条成立即可」，
       而那正是这份 docstring 漂掉时造成的误读（形态 D）。

      ① 闸前短路 —— 覆盖式串：问句**自己定了范围**，而差的又是范围类槽位
         ⇒ 直接算串。⛔ 即使该槽位原本为空，也不许被继承覆盖。
      ② 闸 —— 自足性：问句里**没有**指代/省略词 ⇒ 它什么都没省略
         ⇒ 任何继承都解释不通 ⇒ 算串。
         🔴 顺序要紧：③ 对自足问句**永远到不了**。第一版把 ③ 放在 ② 前面，
            于是「这个月毛利多少」继承 history 的门店/菜被判成合法 ——
            那正是客户抱怨的那个形状（详见 ② 实现处上方的注释）。
      ③ 过闸之后 —— 空槽补全：不带 history 时这个槽位**是空的**
         ⇒ 问句确实没说，继承是补全 ⇒ 解释得通。
      ④ 兜底：走到这里说明问句含指代 ⇒ 解释得通。

    ⛔ 这里**没有**「继承之后值没变」那一条。`slot_diff` 只返回**不相等**的
       槽位，`before == after` 根本进不到这个函数 —— 写上去就是一条永不触发
       的空规则。⚠️ 谁想把它加回来，先让 `slot_diff` 返回相等项。
    """
    empty = (None, "", (), [], False)
