"""Tests for services.materialized_analytics.daily_void aggregator.

New restaurant analytics dimension: 撤单率 / 撤单稽核 (greenfield). Mirrors
test_revenue_report_daily_order_type_meal_aggregator.py's mock pattern —
verifies:
...
from smartbi.services.materialized_analytics.daily_void import (
    materialize_daily_void,
...
async def test_returns_affected_row_count_from_pg_status():
    ...
    affected = await materialize_daily_void(
        pool, "DEMO_REST",
...
async def test_upsert_sql_executed_with_correct_params():
    ...
    for call in conn.execute.call_args_list:
        args = call[0]
        if args and _AGG_DAILY_VOID_UPSERT_SQL in args[0]:
            upsert_call = call
            break
...
async def test_pg_no_match_returns_zero():
    """When fact_pos_void has zero matching rows, UPSERT inserts 0."""
