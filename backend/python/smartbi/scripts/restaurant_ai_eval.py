...
# ── 日期敏感用例：本周是否真的还没有数据 ────────────────────────────────
# 「周初当前周期暂无数据 → 不泛化补数、主动给相邻周期」这条行为只在**本周确实为空**
# 时才该触发。demo 数据每日刷新、覆盖到**昨天**，而「本周」= 周一..今天，所以：
#
#   周一   → 本周只有周一这一天，而当天数据还没落 → 本周为空 → 应走"暂无数据 + 相邻周期"回落
#   周二~日 → 本周已含至少一个完整日 → 有数据 → 应给正常经营分析，**绝不能**谎称没有数据
#
# 之前这条用例把"暂无数据"写死成期望，等于只有周一能通过，其余六天必挂
# （2026-07-28 周二实测：本周 30 家店 ¥285,776，系统正确给出分析却被判失败）。
# 现在按星期切换断言：两个分支都在真正地检查行为，而不是靠运气对齐日期。
#
