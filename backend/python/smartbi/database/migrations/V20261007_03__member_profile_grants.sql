...
-- analytics tables. V20261007_01/02 created dim_member, fact_member_recharge,
-- agg_member_tier, agg_member_gender, agg_member_birth_month,
-- agg_member_recharge_daily — all owned by postgres (the migration runner);
-- without this grant the app role smartbi_user gets "permission denied for
-- table" on INSERT — hit at deploy time by both the demo-data loader
-- (dim_member / fact_member_recharge) and materialize_member_* (agg_member_tier
-- / agg_member_gender / agg_member_birth_month / agg_member_recharge_daily).
-- The runner does NOT auto-grant; this mirrors V20261005_03__void_grants.sql
-- (which itself mirrors V20260428_03__b_silver_grants.sql and
...
GRANT SELECT, INSERT, UPDATE, DELETE ON agg_member_gender           TO smartbi_user;
...
-- dim_member / fact_member_recharge have BIGSERIAL ids; the four agg_* Gold
