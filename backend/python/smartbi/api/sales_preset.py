...
def _money_fields(taxable_amount, tax_amount) -> dict[str, float]:
    """Expose report money fields using the persisted sales tax split.

    SalesOrder.total_amount / SalesOrderItem.unit_price are stored as untaxed
    amounts. Use tax_amount / item tax_rate instead of re-dividing aggregates,
    otherwise mixed rates or already-split orders get distorted.
    """
    ...
    return {
        ...
        "data": {
            ...
            **_money_fields(total["taxable_amount"], total["tax_amount"]),
            ...
            "unpaid": (
                float(total["taxable_amount"] or 0)
                + float(total["tax_amount"] or 0)
                - float(total["paid"])
            ),
            "daily": [
                {
