...
async def gross_margin(request: Request, days: int = Query(30, ge=1, le=365)) -> Dict[str, Any]:
    ...
    # ⛔ 这段原来是内联的第二份 per-dish 成本口径 —— 它自己的注释就承认了
    #    ("recompute ... here directly from the same join logic").
    #    发现层的毛利规则需要同一份数字, 再抄一遍就是第三份, 所以搬进
    #    `gold/restaurant/dish_margin.py` 供两边共用。本端点响应形状逐字不变。
