...
function run(filePath) {
  return spawnSync('node', [SCRIPT], {
    env: {
      ...process.env,
      CODEX_TOOL_INPUT: JSON.stringify({ file_path: filePath }),
      CLAUDE_TOOL_INPUT: '',
    },
    encoding: 'utf8',
  });
...
const t7 = spawnSync('node', [SCRIPT], {
  env: { ...process.env, CODEX_TOOL_INPUT: JSON.stringify({ old_string: 'x' }), CLAUDE_TOOL_INPUT: '' },
  encoding: 'utf8',
});
assert.strictEqual(t7.stdout.trim(), '', `T7 fail: missing file_path should be silent`);

// T7b: legacy CLAUDE_TOOL_INPUT fallback remains supported.
const t7b = spawnSync('node', [SCRIPT], {
  env: {
    ...process.env,
    CODEX_TOOL_INPUT: '',
    CLAUDE_TOOL_INPUT: JSON.stringify({
      file_path: 'frontend/CretasFoodTrace/src/screens/processing/LegacyScreen.tsx',
    }),
  },
  encoding: 'utf8',
});
assert(t7b.stdout.includes('UX Advisory'), 'T7b fail: legacy CLAUDE_TOOL_INPUT should still trigger advisory');
