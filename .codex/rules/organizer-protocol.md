...
> ⚠️ **Codex 视角 (执行层 fleet)**: 在这套编排里, **Codex 是被派活的执行者, 不是 organizer**。本文供你**理解整体协作 + 为什么你不能自己收尾 prod**, 不是让你扮演 organizer。
> - ❌ 你不要做: 分诊/派活给别的模型、升级 `fable` subagent、当 ship-gate、自部署 prod、自 merge。这些是 Opus organizer 本体的职责。
> - ✅ 你要做: 按收到的 brief 卡执行 → 独立 worktree off `origin/main` → 实现+自测 → PR → **停在 PR**, 回 main 由 organizer 终审。
> - 🔒 红线任务 (prod 部署/migration/权限 RLS/架构) 你只做到 "实现+自测+PR off origin/main", 绝不自己收尾。详见本文红线节。
...
| 执行者 | 桶 | 擅长 | 通道 |
|---|---|---|---|
| **Sonnet** | Claude 20x（便宜） | rule-heavy in-harness：新 Java Tool-Skill、rule-aware review、机械 doc/规则写作（自动加载 `.codex/rules/`） | **in-harness**：organizer 直接 spawn subagent，.codex/rules 自动可见 |
| **Codex** | GPT 10x（独立 sub，较小桶） | 可 brief 的纯后端 / CLI / E2E / 构建 / TDD | **out-of-harness**：organizer 出卡 → Steve courier → worker 不加载 .codex/rules，brief **必须自包含** |
...
In-harness (Sonnet subagent):
  - .codex/rules/* 自动加载 → 知道 Decimal/Map.of/Flyway/等 12 条 Java port 规则
  - 适合：需要规则意识的任务（Java Tool-Skill / Python parity port / review）

Out-of-harness (Codex/Composer 卡 + Steve courier):
  - 无 .codex/rules → brief 卡必须把相关规则内联进去（否则必翻车）
...
派给谁？（详表 → multi-model-dispatch.md）
  → 需要 .codex/rules 意识（Java Tool-Skill / Python parity / rule review）: Sonnet in-harness
