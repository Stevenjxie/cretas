...
> ⚠️ **Codex 视角 (执行层 fleet)**: 本文描述 Opus organizer **如何把活派给你**(及 Sonnet/Composer)。你是接活的一方, 读它是为了理解自己的边界, **不是让你按它去派活/选模型/升 Fable**。路由决策、effort/orchestration 选择、Fable 5 升级闸全是 organizer 本体的事。你只需: 按 brief 执行 + 隔离 worktree + PR off origin/main + 停在 PR 等 organizer 终审。详见 [`organizer-protocol.md`](./organizer-protocol.md) 顶部 Codex 视角说明。
...
Sonnet         = Claude 侧默认执行层 (便宜 20x, rule-heavy in-harness, 自动加载 .codex/rules)
...
**两派发通道**:
- **In-harness**（Sonnet subagent）：organizer 直接 spawn，`.codex/rules/*` 自动可见 → 适合 rule-heavy 任务（Java Tool-Skill / Python parity port / rule-aware review）
- **Out-of-harness**（Codex/Composer：organizer 出卡 → Steve courier）：无 `.codex/rules`，brief 卡**必须自包含**相关规则摘要（否则必翻车）→ 适合可完整 brief 的纯执行
...
| **规则重 in-harness 执行**（新 Java Tool / Flyway 编号检查 / rule-aware code review / Python parity port 机械修复） | **Sonnet in-harness** | `.codex/rules/*` 自动可见 → 不会因缺上下文翻 12 条 Java port 规则；比 Opus 省 20x 额度 |
| **Java AI Tool-Skill 意图路由 · Python↔Java parity port（首次判断/架构）** | **Opus**（或 Opus 写死严格 brief 后给 Sonnet/Codex） | 有 `.codex/rules/ai-intent-tool-skill-architecture.md` / `python-java-port.md` 硬规则(Decimal/Map.of order/Lombok null/HALF_UP 等 12 条), 易踩; 首次需判断力 |
...
> 模型名以 Steve 当前工具箱为准(Cursor Composer / OpenAI Codex / 本 Claude chat / Claude Sonnet subagent)。名字变了路由逻辑不变 —— 关键是"执行 vs 判断"分层 + 有没有 `.codex/rules` 上下文。
...
| 场景 | 审查者 |
|---|---|
| 例行 review（无风险改样式/lint/补测试/无红线依赖） | **Sonnet**（rule-aware in-harness，.codex/rules 自动可见，省 Opus 额度）|
...
- ❌ Out-of-harness Codex/Composer brief 卡不内联相关 `.codex/rules` 摘要 → Java parity/Tool-Skill 规则缺失必翻车
