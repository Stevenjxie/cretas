...
| 内容 | 所在服务器 | 路径 | 域名 |
|------|-----------|------|------|
| Java 后端 | **47** (新) | `/www/wwwroot/cretas/` | server-local `localhost:10010`; public direct access closed |
| Python 服务 | **47** (新) | `/www/wwwroot/cretas/code/backend/python/` | server-local `localhost:8083`; public direct access closed |
...
**Phase 3 网络规则 (2026-04-11)**: 47 上的后端端口 `10010/10011/8083/8084` 不再对 `0.0.0.0/0` 开放，仅放行 Nginx 网关 `139.196.165.140/32`。开发机直连 `47.100.235.168:10010` 超时是预期现象，不代表服务故障。开发机访问 Java 后端请使用 `ssh -L 10010:localhost:10010 root@47.100.235.168`。
...
```bash
# 健康检查 (在 47 服务器本机跑，因为后端端口对公网关闭)
ssh root@47.100.235.168 "curl -s http://localhost:10010/api/mobile/health"   # 生产 Java
ssh root@47.100.235.168 "curl -s http://localhost:10011/api/mobile/health"   # 测试 Java
ssh root@47.100.235.168 "curl -s http://localhost:8083/health"               # 生产 Python
ssh root@47.100.235.168 "curl -s http://localhost:8084/health"               # 测试 Python
...
详见 `.agents/skills/deploy-backend/SKILL.md` 和 memory 里的 `feedback_deploy_pipeline.md`.
