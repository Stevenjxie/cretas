  ...
  for (let i = 0; i < picks.length; i++) {
    ...
    if (isRealClk(b)) {
      ...
      if (cb && num(cb.boxCount) > 0) {
        ...
        // 成本拆分 = 料+工+调+包 (含 packagingCost — 审计补; pkg>0 的批次正向验证包装成本)
        const compSum = (num(cb.rawMaterialCost) ?? 0) + (num(cb.laborCost) ?? 0) + (num(cb.seasoningCost) ?? 0) + (num(cb.packagingCost) ?? 0);
        if (total != null) approx(compSum, total, 0.05, `${tag} 成本拆分 = 料+工+调+包${num(cb.packagingCost) > 0 ? '(含包装' + cb.packagingCost + ')' : ''}`);
