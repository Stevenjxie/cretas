...
> ## ▶️ 已激活 —— 范围收窄（2026-08-05 owner 二次拍板）
>
> **只下画布里的 BOM 抽屉。BOM 菜单页保留不动。**
>
> 起因：owner 从画布上「在右侧配置 BOM →」进去，落到旧抽屉，撞上一个调料保存不了的既有缺陷，
> 反馈「先从前端把旧 BOM 的页面下掉，避免操作失误」。确认范围后收窄为只下抽屉。
>
> **硬约束（已实测）**：`openPackagingEditor` 仍调 `openBomDrawer(skuId,'PACKAGING')` ——
> **包材编辑只有抽屉这一个入口**。所以 Task 1（画布上的包材弹窗）必须先做完，
> 否则拆掉抽屉 = 包材只能看不能改。
>
> **本次执行**：Task 1（包材弹窗）+ Task 3（下抽屉）。
> **本次不做**：Task 2 的全局开关（范围只剩抽屉一处，不需要跨文件开关）、Task 4（BOM 菜单页保持原样）。
