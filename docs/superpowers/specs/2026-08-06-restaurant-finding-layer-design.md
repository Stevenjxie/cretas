...
> **prod 实测（2026-08-06，只读，用与实现逐字相同的 SQL）**：
> `cur_n=25 / base_n=13 / 交集=13 / 并集=25` → Jaccard 恰好 0.52。
> 交集等于 base 全量，说明 base 的 13 种是 cur 的 25 种的**真子集** ——
> 08-01 新出现的 12 种是净增，不是替换。闸 A 实测 `days=21` 通过，
> 印证了「闸 A 单独挡不住今天这个 case」。
...
```
变质损耗近 7 天 ¥26.4 万，占全店损耗 ≈37%
```

> **数值会逐日漂移，不要写死。** `agg_restaurant_daily_ops` 的 `max(date)` 就是当天，
> 数据仍在写入 —— 2026-08-06 当天先后两次实测得到 ¥264,978.01 / 36.9%(¥263,963.51)。
> 验收断言应比对「同一时刻的 SQL 直查」而非硬编码金额。
>
> ⚠️ 本文早期版本写的「¥291,112 / 37.2%」是**探索期用 `date >= CURRENT_DATE - 7`
> 算出来的 8 天窗**。上线规则用 `>`（7 天），差的正是 07-30 那一天，金额差约 ¥2.6 万。
> §5.1 的边界修正确实会改数，不是纸面洁癖。
