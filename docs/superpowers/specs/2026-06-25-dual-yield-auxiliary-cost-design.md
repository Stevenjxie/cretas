...
**已定（Steve 2026-06-25「按推荐做」）**：
- `ProcessAuxiliaryRate` 配置粒度：**按 (产品×工序)**（产品非标、辅料差别大）。
- 辅料单价录入入口：**工序配置页加一列**（贴现有工序配置，不另开维护页，最轻）。
- 核对差异阈值：**默认 5%**（对齐 settlement variance），**做成工厂级可配**（小成本，留弹性）。
- 出 PR 顺序：**A修 → 段1双出成率 → 段2辅料**（§10），分三批，各自 PR off origin/main，Opus 终审从 main 部署。
