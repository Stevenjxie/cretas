...
## 7. 产品决策（E，Steve 已拍板 2026-06-04）
- **E1 = 保留** 计划数量字段（不动）。
- **E2 = 保留"指派主管"(大组长)，但改为非必填**（重心在工序小组长报工，该负责人可选不强制）。→ 实现：`production/plans/list.vue` 新建表单去掉该字段 required 校验 + 标"(可选)"；后端若有 @NotNull 则放宽。归 production/plans 线。
- **E3 = 厘清"计划日期 vs 批次日期"**（查实：`ProductionPlan` 同时有 `plannedDate`[必填,计划日期] + `batchDate`[可选,批次日期]，表单/表格都显示，语义重叠混乱）。→ 实现：① 先查 `batchDate` 是否被 create-batch / 任何逻辑实际消费；② 若**未被消费** → 移除 `batchDate`，只留一个日期（"计划生产日"，默认今天，并入 A5）；③ 若**被消费** → 明确标注（计划日期=计划生产日 / 批次日期=实际转批次日，加 tooltip 说明）。归 production/plans 线。
- **C2 已确认**：右面板"工序流程(上)/可添加工序(下)"上下堆叠 → 改**左右两栏对照**。
