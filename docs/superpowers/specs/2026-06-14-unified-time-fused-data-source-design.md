...
### 1.1 为什么 Phase 2A 之后还是这样(半截迁移的教训)

"我们不是做过 Java→Python 吗?"——做了,但**只做了读半边,且停在了易做边界**:
- **范围窄**: Phase 2A 只 port 了 50 个**纯读/分析**端点(要求字节级 parity,见 [[python-java-port]] 的 12 条硬规则)。摄取(上传 flow)、运营同步、`dynamic_data` per-upload 动态分析核心、Java AI 取数器**从不在范围内**——因为它们**有状态/强事务/带确认门控+RBAC**,不像纯读函数能 parity 地搬,风险高被推迟。而"选数据源/只读最新"的痛点恰好长在这没搬的核心里。
- **收尾欠债**: 即便搬走的读端点,Phase 2A 只在 nginx/HTTP 层切到 Python,**没删 Java 服务** → 它们成"THIN 僵尸"(外部死、`enrichUnifiedDashboard` 内部还调)。
- **半截最糟**: 把一个**共享数据层**的"读半边"搬 Python、"写/摄取半边"留 Java = **split-brain**(两语言都写同一批 smartbi 表),正是审计冲突的总根。

**铁律(本次纪律)**: **共享数据层的语言迁移,要么别动,要么连"写"一起搬到底——只搬"读"会卡在最糟的半截。** Phase 2A 当年为 parity 安全只搬纯读是务实的,代价就是今天的 split-brain。本次"数据处理整格归 Python、一次到底",就是为了不再半截。
