...
### ① 工序带「调味类型」(master data)

- `WorkProcess`(工序主数据)加 `seasoningType` 枚举:**`NONE`(无) / `INJECTION`(注射) / `COOKING`(熟制)**。
- 在**工序管理页**(`web-admin/src/views/system/work-processes/index.vue`)设置。因每个 SKU 熟制是独立 WorkProcess,在工序上标一次即可。
- 该标签决定:配调料时出哪种表单;报工时是否要记锅数。
- 名字正则(熟\|卤\|煮…)降级为**兜底**(seasoningType 未标时的旧行为),不删,避免存量工序回归。

### ② 调料配方按工序(核心数据 + UI)

**数据模型:**
- `BomSeasoningItem` 加 `work_process_id`(nullable,迁移期兼容)。每条调料明细归属 (SKU-recipe × 工序)。`section` 变为冗余(可由工序 seasoningType 推导),保留兼容读。
- **每道工序的锅序/注射参数**需要一个 per-(recipe × 工序) 的家。新增轻量表 `bom_process_seasoning`(或等价):`(id, recipe_id, work_process_id, subsequent_pot_ratio, injection_rate, notes)`,一行/（SKU-recipe × 工序）。`BomSeasoningItem` 明细行按 `(recipe_id, work_process_id)` 归组到它。
  - 熟制工序:用 `subsequent_pot_ratio`(第二锅起比例)。
  - 注射工序:用 `injection_rate`(注射率/注射量口径,见 ③ 澄清)。
...
- 每道工序按其 `seasoningType` 出对的表单:
  - **熟制**:锅序(第二锅比例)+ 调料明细(每种卤料:名称/每kg用量/单价/是否计入成本〔老汤高汤不计〕)。
  - **注射**:注射量/注射率 + **注射内容明细**(盐水/添加剂…,每种注射物 + 用量)。〔Steve 澄清:注射配方 = 配"注射多少量 + 注射什么内容",内容可配,不是固定物〕
  - **普通(NONE 但用户想投料)**:只填调料明细。
  - **无需调料的工序**:留空(=0),**不强制**。
...
注射是专门工序。注射配方配两件事:
1. **注射多少量** —— 注射率(相对原料重量的 %)或注射量。沿用 `injection_rate` 字段承载"注多少"。
2. **注射什么内容** —— 注射内容明细(盐水 / 添加剂 …),每种注射物按用量(沿用注射段 `BomSeasoningItem` 明细行,dosage 口径)。内容可配,不是固定某种物质。

成本:`RecipeCostCalculator` 注射段现有算法 = `injectionRawKg × Σ(注射内容 dosage × 单价)`,与此语义一致,复用。

### ④ 报工锅数泛化

- 报工识别"这步要记锅数/是调味步"从**硬编码 `shuzhi` + 名字正则**改成**读该工序的 `seasoningType`**:
  - `seasoningType == COOKING` → 显示锅数录入(几锅 + 逐锅原料 kg)。
  - `seasoningType == INJECTION` → 显示注射量录入(无锅数)。
  - 名字正则保留为 seasoningType 未标时的兜底。
- 需把 **`workProcessId` 透传进 clerk 报工路径**(现 clerk 路径按 StepEntry 的 processName/Order 认工序,workProcessId 只用于自定义字段校验)。让报工能读到该步的 seasoningType + 该步的锅序参数。
- 报工记"起了几锅" → `computeSeasoningCost` 按**该工序的**锅序参数算(见 ⑤)。
- web-admin `ProcessDataTable.vue`:锅数 expander 的 `isShuZhi`(`processCode === 'shuzhi'`)改为按该步 seasoningType 驱动。
...
- `WorkProcess.seasoningType`(Flyway 加列 + 枚举)。
- `BomSeasoningItem.work_process_id`(Flyway 加列)+ `bom_process_seasoning`(per-工序 锅序/注射参数表)。
- 迁移:存量 `BomRecipe` header 的 pot 参数 + `section` 明细 → 映射到该 SKU 的熟制/注射工序组(F006 为空,近似 no-op;写迁移但低风险)。
- `computeSeasoningCost` 按工序读;`isSeasoningStep`/锅数识别读 seasoningType;`workProcessId` 透传进 clerk 报工路径。
...
### Slice B — 调料配方 tab 按工序 UI + 工序类型设置
- 工序管理页加 `seasoningType` 设置。
- 调料配方 tab 重构:按工序分组 + 按类型出表单(熟制锅序 / 注射量+内容 / 普通)+ 软提示 + 空状态导航。
- 复用 Phase 1 的防呆规范(4 位一体错误 / dead-end 导航)。
- **验证**:headed 走查——建 workflow → 调料配方页按工序列出 → 各类型表单正确 → 留空不强制 → 保存。

### Slice C — 报工锅数 UI 泛化
- `ProcessDataTable.vue` 锅数/逐锅原料录入从 `processCode === 'shuzhi'` 硬编码改为按该步 seasoningType 驱动。
...
- `section`(INJECTION/COOKING)保留兼容:新写入按工序 seasoningType 决定段,旧读兼容。
- 存量 `BomRecipe` header 的 `subsequentPotRatio`/`injectionRate`:迁移时若 SKU 有唯一熟制/注射工序,搬到对应工序组;多/无则留 header 默认 + 记 warning(不静默丢)。
...
- 报工锅数录入不再依赖 `shuzhi` 命名,按 seasoningType 驱动。
