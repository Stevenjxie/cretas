...
### P2.2 反回扣 理论vs实际 GAP/损耗 — 评估后**不建** (2026-07-10)
理论 vs 实际 GAP 的价值被**成本卡理论侧的根本残缺**卡死: 中餐只有加权/聚合理论毛利, 无可靠逐菜/逐食材理论 (用量测不准), 名称解析仅部分覆盖 → 差额分不清**真漏点 vs 理论没算全**, 投入高 (要建 per-dish 销量 + 菜→档口映射 + 覆盖率门) 产出弱, 还带 Fable 说的"按构造冤枉人"误告风险。**ROI 负, 任何期都不建。**

**反回扣深化改走纯实际趋势** (绕开残缺理论):
- **已建 (本期 P2.1)**: 供应商价格异常 — 采购端回扣, **零理论依赖**。
- **候选 (待验证, 非本期)**: **实际成本率同比环比报警** ("成本率环比 +4% → 去查", 邓总核心, 不指名不冤枉人)。前提: 打通"盘点→纯实际用料成本 (期初+采购−期末)"链路 — ⚠️ `finance_summary` 现成本**可能来自同一条理论 ETL** (需核实), 那样"实际"又变理论; 真独立实际信号只有 领料/盘点/损耗。等真客户 (邓总/青花椒) 有真盘点数据再评估。
