...
| W9-AI-RBAC | 🔴 #704系统性收口: 全AI写tool RBAC(中央choke point优选根治所有写tool+6执行路径, 或逐tool推广) | Opus红线 | - | bg subagent | fix/w9-ai-rbac-systematic | ai/tool执行链 | 🟡 in-flight | 停PR | Fable发现的唯一系统性安全缺口. 多租户ERP真洞(AI删客户/订单/发票/转账无鉴权) |
...
| REPORT-MODEL-REDESIGN | 🔴 **核心: 报工模型重设计(Steve拍板, 取代#690本末倒置)**: 工序从强制→optional, 报工粒度从"工序属性"升级为"计划级模式". **免工序报工(默认ON)**=批次级两点(领料+产出, 无人工, 人工登下一期), 出成率=产出/领料, 成本只含料. **逐道(可选, 需配工序)**=工序级(#690 reportingRequired变细粒度选项). 没配工序=强制免工序报工. 责任人头尾各一或一人兼. ux-flow Phase1门控过(UX Flow Analysis产出). | Opus keystone+web/RN | - | bg subagent | feat/skip-process-reporting-be | ProductionPlan/spawn/报工/成本 | 🟡 后端keystone在飞 | 停PR | 后端定契约→web(计划免工序开关+SKU工序optional)+RN(任务列表2task+领料/产出报工屏无人工). 向后兼容: skip_process_reporting DB default false(现有逐道不变), createPlan API默认true(新计划两点) |
