...
> 🆕 **CIG-PHASE-A 程序 (2026-06-10, brainstorm→spec v2(4-critic审计)→plan→subagent-driven)**: "全局所有图表都有 图表+洞察 + 全飞轮". 拆 4 阶段. spec `docs/superpowers/specs/2026-06-10-chart-insight-foundation-phase-a-design.md` / plan `docs/superpowers/plans/2026-06-10-chart-insight-foundation-phase-a.md`. 决策: 全~44面+补族 / Tier1主+Tier2-on-null自动 / 跨租户模板(签名去factoryId) / 富 Phase A. **✅ Phase A 地基 U1-U6 全 shipped+merged+部署** (origin/main 8d87d18e4): U1 飞轮硬化8项(permission_tier服务端推/签名去factoryId跨租户/safe_fill/poison-¥全字段/budget fail-closed/迁移V20260928_01 live验) + U2+U3 三族补全(KPI¥守卫)+deriveChartMeta扩词表 + U4 useChartInsight composable(2-slot限流, gate抓semaphore泄漏已清) + U5 上传meta死接线修(helper单测) + U6 现有6图迁composable+接回Tier2. **+2 flywheel hotfix(real-path curl抓, mocked pytest漏): lookup JSONB列(suggestion_tpl非列) + LLM JSON robust parse(glm-5.1 markdown/reasoning) + max_tokens 500→2000(reasoning模型空content).**
> **验证状态(诚实)**: ✅ **Tier1(规则0token)全工作**(prod headed: 销售趋势/产品类别/门店排行 3条数据驱动, U6迁移无回归) ✅ **Tier2 LLM生成clean正确洞察**(prod curl: "堂食营收占比达62.0%,是外卖的1.6倍", 无{slot}泄露——禁用Tier2的bug已修). ⚠️ **飞轮收敛未达**: LLM返填充后字面句(非{slot}参数化模板)→validate拒capture→0行入库→不收敛到0token(洞察正确但每次重生成). 根因=LLM prompt-tuning(让模型返{topName}占{topShare}模板而非填充句), 是follow-up. ⚠️ U5上传面headed验deferred(demo租户0上传).
> **下一步(未启动)**: ①飞轮收敛prompt-tuning(让LLM返参数化模板) ②Phase B/C/D rollout(44面, per-机制adapter). SDD全程: brainstorm→spec v2(4-critic对抗审计抓permission_tier红线BLOCKER等)→plan→subagent-driven(Wave1‖→Wave2‖→Wave3)+Opus红线gate+real-path验真(多次抓"单测过prod不fire").
...
| CI-U5 | Phase1 U5: 集成+部署(✅) — 但 **demo 接线前提被证伪**(见下) | Opus编排 | - | - | integrate/ci-phase1→main | demo看板+slide | 🔴 重定向→CIG-GOLD | - | 🔒 4分支merge+三服务部署✅. **但 2026-06-10 新 organizer 接管求证(后台 read-only agent live prod 验): Tier1 在 prod 一处都不 fire** → ①U1 后端 setMeta 是死代码(`buildDynamicChartConfig*` 0 生产调用) ②live 上传流不发带 meta 的 charts[](`UploadFlowResult` 只单 chartConfig) ③前端 charts 在 `analysis.ts` chartsLane 客户端构建从不挂 meta ④两 demo 租户 0 上传(`totalElements:0`) ⑤demo 实走 `RestaurantGoldGrid`(自有 revenueInsight, 从不碰 chartInsight.ts/meta). Tier2 端点工作但孤儿. 前 chat gate 被单测(手搓带 meta 的 chart)骗过=同 U1/U4 接线盲点. **Steve 决策**: 接进 gold 驾驶舱(Option A)→CIG-GOLD |
| CIG-GOLD | chart-insight 接进餐饮 gold 驾驶舱(客户端构 ChartMeta, 绕死后端 meta 路径) | Sonnet→Opus | (锁死) | 单subagent+Opus | merged main | RestaurantGoldGrid.vue + chartInsight.ts + ChartInsight.vue + spec.ts | ✅ DONE+headed验 | - | 🔒Opus gate+headed live验 prod R_SSW_DEMO. **决策调整**(real-path验暴 Tier2 LLM slot-fill bug {topChannel}不填充+偶 null)→Steve定**两卡都走 Tier1 干净**: 门店排行(BAR)+渠道占比(BAR)→Tier1 RANKING 瞬时"数据驱动"(%/倍 RBAC-safe). **headed验 PASS**: 门店卡显"蜀三味·锦里旗舰店…1.7倍;占合计24.1%"(渠道无数据=honest-null). 阈值1已回滚(demo Tier1-only 无 Tier2 caller). Tier2 端点+fetchTier2Insight 留 Phase2(slot-fill bug 待修). 工厂侧 demo=RN 截图→chart-insight **仅餐饮**(全模块/上传/工厂 web=Phase3) |
| CIG-DASH | chart-insight 扩到驾驶舱 销售趋势+产品类别占比(Steve "其他模块图表也上") | Sonnet→Opus | (锁死) | 单subagent+Opus | merged main | Dashboard.vue | ✅ DONE+headed验 | - | 🔒Opus gate+headed live验. trendInsight(time→TREND/类目→RANKING)+categoryInsight(归一 BAR→RANKING). **gate 抓 real-path bug(subagent 漏)**: 后端发 legacy `{data,xaxisField,yaxisField}` 非 `{xAxis,series}`→直读为空→null; 修=先 `normalizeLegacyChart`(同 initCharts)。**headed验 PASS: 驾驶舱 3 条数据驱动**(销售趋势"趋势平稳0.1%"/产品类别"招牌卤猪蹄…2.9倍占19.8%"/门店排行…1.7倍; 渠道无数据=honest-null)。%/倍 RBAC-safe。eb3184396 已部署 |
| CIG-DECK | 参赛 deck: AI 成本曲线/技术护城河 slide + 重截图驾驶舱 | Opus自做 | - | inline | merged main | competition/{slides.html,screenshot_slides.py,pptx} | ✅ DONE | - | 新 slide s4b(规则优先×自蒸馏飞轮 cost-decay SVG+三层 resolver, on-brand)→deck 8 页(renumber /7→/8)。restaurant-03-dashboard.png 重截为含 3 数据驱动条的驾驶舱。pptx 用 /b/anaconda3/python screenshot_slides.py 重生成(839KB/10 PNG)。8f148199e 推 origin/main。assets/screenshots gitignored 仅 pptx+slides.html commit |
| CIG-INSIGHT-FMT | 经营驾驶舱 AI 智能洞察 markdown 渲染混乱修复(Steve 反馈) | Opus自做 | - | inline | merged main | Dashboard.vue + cleanInsightText.spec.ts | ✅ DONE+headed验 | - | 接 fix/dashboard-insight-markdown(原 strip **/##/>/`)。3层根因逐个 headed 验: ①cleanInsightText 加 bullet/注 换行 ②hoist 到组件作用域+aiInsights computed 应用(cached 洞察 display-time 清洗) ③**parseInsightCitations split 含\n→丢 whitespace-only 段→重新黏行**, 去掉 split 的\n(.insight-content 已有 white-space:pre-line)。headed PASS: 核心结论/2 bullet/注 各占一行(5换行)。23 单测。**已部署 web-admin 139:8086** b7bc81b06。坑: Dashboard.vue 该函数行含 nbsp(\xa0)致 Edit 屡 mismatch; python heredoc 又把 \n 写成字面换行→git checkout 重来用 Edit 锚定干净 backtick 行。浏览器 stale bundle 缓存致多次"部署了没变"误判(实为 index no-cache 但 MCP 浏览器缓存; cb 查询+清 cache 后验真) |
...
| — (CIG-GOLD + CIG-INSIGHT-FMT 已合并部署, smart-bi/components + Dashboard.vue 锁释放) | — | — |
| — (T108/T110 已合并部署,锁释放) | — | — |
