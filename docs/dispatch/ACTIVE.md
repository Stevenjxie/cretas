...
| T119 | 修 AI配工序解析保真(该多少是多少, **不凑数/不重复填充**) | Sonnet→Opus gate | locked | inline | fix/ai-workprocess-parse-fidelity | WorkProcessConfig NL tool prompt + web-admin 工序配置 | 🟡 in-progress | - | 🔒 焯水×4 凑数 bug。gate 严查"忠实转写不 count-forcing"+headed 验 |
| T121 | 工序**多人负责**(级联下拉, 任一负责人可报) — 设计 recon | Sonnet recon→Opus 设计 | locked | inline | (只读) | ProductWorkProcess assigned_to → 关联表 + spawn/报工门控 + 前端级联 | 🟡 recon | - | 🔒 schema+报工权限。任一负责人可报(Steve 定)。recon 摸现状+标与 T119 冲突→Opus 设计→排 T119 后派 impl |
