...
| WAVE7 | Tier1 第2批: #709 SP6 RN收货屏(WHPurchaseReceiveListScreen注册进WHInboundStack非孤岛+待收单防呆, merged, →OTA) / #710 SP10中报价(去stub真读getMidQuoteById+修taskId当sampleId bug+confirm状态机CALCULATED→CONFIRMED, merged) / #711 撤回快速通道(本人+无下游消费[G1不绕]+5min窗口, G1/G2/G3保留只免审批人, Flyway V20261016_02, merged) / **#712/#23 并发测试发现真bug→W8修** | Sonnet×4 | - | bg subagent | merged(709/710/711) | (释放) | 🟡 3 merged, W8修bug在飞 | #709/710/711 | **🔴W7-d发现BUG-SP1-NEW-ROW**(真并发bug): WipInventoryServiceImpl.postSemiOutputLedger new-row路径无并发保护→两线程同时insert重复intermediate_batch_no(H2库存翻倍/PG约束抛异常无retry静默500+WIP漏记); existing-row悲观锁+SP2扣减正确只new-row漏. **W8(Opus)修**: retry-into-existing-path或upsert+@Column(unique)+测试翻assert-fixed. #712(失败测试态)不merge, W8 PR取代. Java(710/711+W8+V20261016_02)待批量部署+RN OTA(#709) |
