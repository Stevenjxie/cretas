...
|---|---|---|---|---|---|---|---|---|---|
| FIXB-BE | 六扇门修复批·组1 backend(BUG-2报工联动task/批次DTO batchSourceType/调拨实收/撤回显姓名) | Sonnet | (锁死) | subagent worktree TDD | fix/liushanmen-fixb-backend | YieldReportServiceImpl+ProcessingBatch DTO+TransferController/Service+ReportReversalLog DTO(+Flyway if needed >V20261011_22) | 🟡 in-flight | - | 🔒红线(任务状态/库存) Opus gate |
| FIXB-WEB | 六扇门修复批·组2 web(报损页数据源核对/入库异常页核对/three-price入口) | Sonnet | (锁死) | subagent worktree | fix/liushanmen-fixb-web | web-admin warehouse/wastage-reports + procurement/exceptions + rd/quotations three-price + router/menu | 🟡 in-flight | - | - |
| FIXB-RN | 六扇门修复批·组3 RN(BUG-3领料整行可点/报损批次选择器) | Sonnet | (锁死) | subagent worktree | fix/liushanmen-fixb-rn | RN MaterialBatchPicker+WastageReportScreen | 🟡 in-flight | - | 完后 OTA(Opus 部) |
| FIXB-CFG | 六扇门修复批·组4 掌中宝semiFinishedOutputCode demo配置+真机验双产出+BUG-2复验 | Opus自做 | - | inline | prod data | work_processes.semi_finished_output_code(F006掌中宝) | ⬜ 等组1-3部署后 | - | 真机 f79c50d6 |
...
| CIG-GOLD | chart-insight 接进餐饮 gold 驾驶舱(客户端构 ChartMeta, 绕死后端 meta 路径) | Sonnet in-harness | (锁死) | 单subagent bg TDD | feat/chart-insight-gold-wire | RestaurantGoldGrid.vue + chartInsight.ts + ChartInsight.vue + spec.ts | 🟡 in-flight | - | 🔒RBAC(revenue 财务脱敏)+部署 Opus gate. 设计: 门店排行(BAR)→Tier1 RANKING 瞬时"数据驱动"; 渠道占比(PIE=PROPORTION Phase2→Tier1 null)→Tier2 LLM"AI生成"→阈值1学习→刷新"已学习"=现场闭环. 工厂侧 demo 是 RN 截图非 web 看板→chart-insight **仅餐饮**(6/13). 完后 Opus gate diff+headed live 验 prod R_SSW_DEMO+设阈值1+部署+成本曲线slide+重截图 |
...
| `web-admin/src/views/smart-bi/components/{RestaurantGoldGrid,ChartInsight}.vue + chartInsight.ts + __tests__/chartInsight.spec.ts` | CIG-GOLD | PR merge 后 (无 FIXB 重叠) |
| — (T108/T110 已合并部署,锁释放) | — | — |
