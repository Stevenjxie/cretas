...
| DEPLOY-TEST | 待部署批 7PR+#683 部 test env 验全链(cashier付款链/空价SO/BOM写); prod 等 main 安静+test验过 | Opus出货闸 | - | bg deploy | ../cretas-deploy-prod | - | 🟡 含#683重部署在飞 | - | test全链验证发现 🔴 **空价SO 500 NPE**(下行#683修). Flyway 3迁移applied✓ settlement_type列✓. cashier/BOM待验. prod未部(等main安静) |
