...
| WAVE2 | ✅ 审计 Tier0 第2批 4 PR 全 merged: #687 报损枚举 / #688 财审闸门 / #689 DTO drop / **#690 报工模型(核心)** | Opus×2+Sonnet×2 | - | bg subagent | merged main | (释放) | 🟢 全merged, --env all部署在飞(含迁移V20261015_01) | #687/688/689/690 | 全 organizer gate过. **#690 报工模型**: reportingRequired放ProductWorkProcess, **不改YieldCalculationService**(报工本就report-driven, 免报工序不spawn→出成率天然两点算=最小爆炸半径), DEFAULT TRUE回填现有行=逐道报字节不变(向后兼容铁律), 全免报422, 41测试. 部署后必**live验向后兼容**(现有F006产品spawn逐道不变)+新免报config+迁移applied. RN(ProductWorkProcessConfigScreen)待OTA. **rhythm: 部署+live验后→Fable审Wave1+2 vs转录→Wave3** |
