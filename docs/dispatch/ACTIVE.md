...
| DEPLOY-TEST | 待部署批 7PR 部 test env 验全链(cashier付款链/空价SO/BOM写); prod 等 main 安静+test验过 | Opus出货闸 | - | bg deploy | ../cretas-deploy-prod | - | 🟡 部署在飞 | - | D死→test env空闲; --env test 不碰prod(避撞另chat chart-insight在main的在飞工作). 完成后核jar标记+test全链 |
