...
| REPORT-MODEL-REDESIGN | 🔴 **核心: 报工模型重设计(Steve拍板, 取代#690本末倒置)**: 工序从强制→optional, 报工粒度从"工序属性"升级为"计划级模式". **免工序报工(默认ON)**=批次级两点(领料+产出, 无人工, 人工登下一期), 出成率=产出/领料, 成本只含料. **逐道(可选, 需配工序)**=工序级(#690 reportingRequired变细粒度选项). 没配工序=强制免工序报工. 责任人头尾各一或一人兼. ux-flow Phase1门控过(UX Flow Analysis产出). | Opus keystone+web/RN | - | bg subagent | feat/skip-process-reporting-be | ProductionPlan/spawn/报工/成本 | 🟡 后端#718 merged部署在飞, web/RN在飞 | #718 | **后端keystone #718 merged**(192测试): report-driven零改yield/cost引擎, 2哨兵task(__MATERIAL_INPUT__/__FINAL_OUTPUT__)cumulative天然=产出/领料. 契约清晰→**web(免工序开关+SKU工序optional)+RN(任务列表2task+领料/产出简化屏隐藏时段人工)在飞**. 向后兼容: DB default false(现有逐道不变)+API默认true(新计划两点)+0工序强制两点+旧3-arg委托skip=false. **⚠️interim**: #718部署后F006新建计划默认免工序报工(他们want), 但RN简化屏(W-rn)未OTA前操作员见领料/产出task仍在逐道屏(时段字段可见未用), 功能不破UI待精化 |
