...
| WAVE5 | Fable发现集成断层修复 5项: W5-a AI路径RBAC注入(Opus红线系统性) / W5-b退货UI收尾(Opus) / W5-c WIP路径404+WastageReport导航(Sonnet) / W5-d打印单据明细填充(Sonnet) | Opus×2+Sonnet×2 | - | bg subagent | 各fix分支 | 生产/退货/打印/AI tool | 🟡 in-flight | 停PR | 各verify-first. 回来gate→部署→Tier0真就绪. **教训: PR终审要验前端真接后端(路径/导航/按钮状态), 不只代码逻辑; build过≠跑通** |
