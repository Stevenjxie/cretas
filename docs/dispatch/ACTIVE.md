...
| DEPLOY-PROD-11 | 后端 --env all 部署 11PR(批8+wave1 3) prod+test; web-admin(#685.vue+批web); RN OTA(#686) | Opus出货闸 | - | bg deploy b069tjif9 | ../cretas-deploy-prod | - | 🟡 后端部署在飞 | - | 部署后: 核jar标记(resolveWorkProcessTask/exportInventoryLedger/assertCanReceive调用点/cancel PENDING_APPROVAL守卫) + test live验wave1(cancel守卫409/导出台账/仓库守卫) + web-admin prod + RN OTA推+设备验WHInventoryCheck走盘点 |
