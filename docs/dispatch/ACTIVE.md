...
| DEPLOY-TEST | ✅ 待部署批 8PR(含#683) test 全链验证 **PASS = prod-ready** | Opus出货闸 | - | bg deploy(v20260611_含683) | (释放) | - | ✅ test验过, prod暂押 | - | **test实证**: 空价SO 200/DRAFT/totalAmount=0(NPE修验)✓ / cashier登录+/approved出纳视图 200(D9 G1)✓ / BOM getRecipe 200(#680 revert读路径不LazyInit)✓ / Flyway 3迁移applied+settlement_type列✓. **prod部署暂押**: deploy-backend只部Java jar(与另chat distill Python正交风险低), 但BIG-AUDIT-WF将出更多Java实现波→批量部(避免多次prod churn). 8PR=#669/673/676/678/679/680/682/683 |
