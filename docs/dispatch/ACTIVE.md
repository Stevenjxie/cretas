...
| WAVE4 | Tier0 最后6项: **5/6 merged**(#696 BUG-R1=审计stale早修#662只清文档 / #697 二次加工web UI(WIP picker :max防超领+SEC标签) / #698 双产出展示(StepYieldDTO补4字段聚合纯加不改yield算)+SP8编码兜底(隐藏级联+诚实alert无假码Rule5) / #699 成本链断点(**真代码bug**: ProductionCostUpdatedEvent只SEMI路径发, FINISHED/legacy(output_kind=null, F006全走)不发→costUnitPrice永不回填→actualCost null. 修=FINISHED分支镜像SEMI发事件unitCost null不发)), W4-c零自主权API在飞 | Opus×3+Sonnet×2 | - | bg subagent | merged main | (释放) | 🟡 5 merged, W4-c在飞 | #696-699 | 全gate过. **成本链数据缺口(非代码, Friday/W3)**: F006所有16工序standard_hourly_rate全null(工价未配)+13报工全SUBMITTED 0 APPROVED(未审批)→端到端成本链要配工价+审批报工才真走通(代码断点已修, 数据缺没伪造). 待W4-c → 批量部署 → **Fable审Wave3+4 vs转录** → Tier0收官 |
