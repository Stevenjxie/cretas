...
| GATE-FULLFLOW | GPT/Codex 第三轮 prod F006 完整DEMO模拟(`2026-06-12-fullflow-e2e-simulation.md`) gate | Opus | - | inline 复核 | 纯验证 | docs/audits | ✅ gated | - | **4阻断项 gate 判定**: ①SO-PRICE-0 **✅已修**(根因=遗留AUD3_PROBE审计定价策略TIERED100%折扣毒化所有≤100量F006单, organizer disable+重验落价68, 真live bug非测试假象) ②MULTI-SO-REL **🟢非bug**(#771代码对遍历sourceOrderIds, GPT用legacy related_orders路径=测试假象) ③BUG-RCV **🟡真gap待Steve定**(收货批次unit_price不从PO行价兜底→null→材料成本丢失, 撞防呆核心仓管不填价; admin手填可绕非硬阻断) ④BUG-MR500 **🟡triage**(/processing/material-receipt 500, /material-batches可替代)。含税三行/单SO回填/两点/金蝶/BOM包材 真实路径坐实 |
