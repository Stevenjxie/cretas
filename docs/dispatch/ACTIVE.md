...
| 771-CLOSE | #771 三拐弯 prod DEMO 链闭环验证(A1多SO合并回填/A2撤回自愈/A3采购两路同源): 建2 DEMO SO→财审→合并计划→转批次→两点报工→审批→SQL坐实cost_unit_price; 撤回→null→重报→新值; 两路netRequired对比 | Sonnet | (锁死) | subagent bg | 无(纯数据验证, 不改码) | prod F006 DEMO数据(remark=DEMO-771-VERIFY) | 🟡 in-flight | - | 铁律: DEMO标记+DEMO料批同价入库不动均价+不删数据(清理清单交organizer)。⚠️Steve拍板(2026-06-11): **测试尤其前端自动化今后→Codex**(Claude慢); 本agent是API/SQL链非前端, 已在飞让它跑完, 若卡死→折入Codex第二轮 |
