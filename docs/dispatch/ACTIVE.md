...
- `AGENT-HOTFIX-SPRING-CTOR-20260720` — `review` — Owner: Codex coordinator — Base SHA: `b587e0ee779136a5fe87f6722a61617fe1a7a0db` — 修复 Restaurant Gross Margin Chat selector 的 Spring 构造注入启动失败并补真实 BeanFactory 回归测试；目标测试 4/4 与只读终审通过，待合入 `main` 后复用可信发布门禁重新部署 Java。
...
- `AGENT-HOTFIX-SPRING-CTOR-20260720`：`RestaurantGrossMarginChatRouteSelector.java`、`RestaurantGrossMarginChatRouteSelectorTest.java`、`docs/dispatch/ACTIVE.md`、`docs/dispatch/archive/2026-07-20-active-history.md`；验收：目标测试、Spring BeanFactory 构造测试、同一 Maven release 生命周期、生产蓝绿 readiness/切流、网关与 Restaurant Agent 只读 smoke。
