...
| REPORT-MODEL-REDESIGN | 🔴 **核心: 报工模型重设计(Steve拍板, 取代#690本末倒置)**: 工序从强制→optional, 报工粒度从"工序属性"升级为"计划级模式". **免工序报工(默认ON)**=批次级两点(领料+产出, 无人工, 人工登下一期), 出成率=产出/领料, 成本只含料. **逐道(可选, 需配工序)**=工序级(#690 reportingRequired变细粒度选项). 没配工序=强制免工序报工. 责任人头尾各一或一人兼. ux-flow Phase1门控过(UX Flow Analysis产出). | Opus keystone+web/RN | - | bg subagent | feat/skip-process-reporting-be | ProductionPlan/spawn/报工/成本 | ✅ **全栈上线+真机E2E DONE** | #718/720/721/723/724/725 | **报工模型重设计端到端live验**: be#718(report-driven零改yield引擎,2哨兵task cumulative=产出/领料,192测试) + web#720(免工序开关默认ON+SKU工序optional+badge) + RN#721(领料/产出简化屏隐藏时段人工). **真机E2E抓3集成bug全修**: #723哨兵分支被totalSteps===0截断 / #724 renderEvidenceBlock TDZ(Hermes静默空白) / #725 TDZ根治(分支移到所有const后+guard早返回, 静态decl-order证明). **真机截图16+24验**: 任务列表免工序批次「领料报工」(无焯水/油炸中间工序)vs逐道「油炸」; 领料报工简化屏(批次+量+投入照, 无时段/早午晚班/出勤人数/人工). audit: 2026-06-11-skip-process-reporting-device-e2e.md. **教训: build✓≠跑通极致案例, TDZ运行时错tsc抓不到, 必须真机跑(Steve坚持adb真跑对)**. 向后兼容: DB default false现有逐道不变+API默认true新计划两点+0工序强制. **人工"登下一期"期间级分摊=独立后续(本批仅诚实null)** |
| TWO-POINT-DEMO | ✅ #690机制两点实证(test): 配中间免报→spawn恰好2task(化冻+气调)中间3道=0, 出成率两点算1000kg→540盒steps只2条无NPE. 证#690真产出两点(变逐道模式细粒度选项) | Opus | - | test 10011 | (释放) | - | ✅ DONE | - | audit: 2026-06-11-two-point-report-flow-test.md. DEMO残留test cretas_db可清 |
| TX-MANAGER-FIX | ✅ #717成本回填tx-manager修复(cost-chain mock E2E发现的隐藏断点): 2监听器硬编码不存在的"transactionManager"bean→回填抛NoSuchBeanDefinitionException(fail-soft之外)→cost_unit_price/actualCost永null(影响所有配工价+料价工厂含prod). 修=primaryTransactionManager. merged部署在飞 | Opus inline | - | merged main | (释放) | - | ✅ merged部署在飞 | #717 | cost-chain E2E链1-4+6✅只链5(event回填)断=此bug. 用mock data跑才暴露(用户指示"别等Friday用mock data跑"的价值) |
| 🏆MILESTONE | **本session总成果: Tier0(16必经)+集成断层(5)+Tier1(11/12)+Tier2/P1(3)= 35代码项全部上线prod验证. PR #684-716(33 merged+1 supersede). 全Opus终审+Fable审计Wave1+2(零弯曲)/Wave3+4(戳穿后端对≠可用→Wave5补5断层)/Wave5/6/7(在飞). 教训固化memory(build✓≠跑通/controller鉴权≠service鉴权). W7-d发现+W8修真并发bug(WIP库存安全)** | Opus organizer | - | - | organizer分支累积30+PR | - | ✅ Tier0+Tier1+部分P1代码done | #684-716 | **剩余(都有依赖/低优先, 非autonomous code blocker)**: ①#26 headed E2E(需小米设备跑RN报工链+e2e-native skill, 焦点session做) ②Tier2/P1剩(#29-31/34-42 多需独立spec/客户/post-launch低优先, 非上线blocker) ③**Friday客户确认3项**(F006工序免报清单/工价配置+报工审批/半成品重量库存vs流水账=数据配置非代码). **组织者分支organizer/2026-06-11-session待Steve确认后merge main**(你说"最后做完再merge") | PR #684-713(30 merged+1 supersede). 全Opus终审+Fable审计Wave1+2(零弯曲)/Wave3+4(戳穿后端对≠可用→Wave5补5断层). 教训固化memory(build✓≠跑通/controller鉴权≠service鉴权). W7-d发现+W8修真并发bug(WIP库存安全)** | Opus organizer | - | - | organizer分支累积30+PR | - | ✅ Tier0+Tier1代码全done | #684-713 | **剩余(都有依赖/低优先, 非autonomous code blocker)**: ①#26 headed E2E(需小米设备跑RN报工链+e2e-native skill, 焦点session做) ②Tier2/P1(post-launch nice-to-have, 非上线blocker) ③**Friday客户确认3项**(F006工序免报清单/工价配置+报工审批/半成品重量库存vs流水账=数据配置非代码). **组织者分支organizer/2026-06-11-session待Steve确认后merge main**(你说"最后做完再merge") |
...
| 周五确认单 | 客户当面确认: G5付款销售方向语义 / PMC+配料员角色(批B缺) / 16位编码SP8 / 财务接API SP11 scope | - | - | - | - | - | 🟢 一页纸已备 | - | **`docs/meetings/2026-06-09-liushanmen/周五确认单-2026-06-12.md`**(4项: 现状+要问+答案落法, Steve 带去当面过)。答案回来→拍板项给 organizer 落任务 |
