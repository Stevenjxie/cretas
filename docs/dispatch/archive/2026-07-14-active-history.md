Warning: truncated output (original token count: 32090)
...
> |---|---|---|---|---|---|---|---|---|---|
> | LSM-FACTORYNUM | 🔒 原料厂号结构化(ManufacturerRegistry 登记表+收货原子录入+web下拉; 厂号=独立属性; RN不碰) | Codex+Opus gate | out-of-harness | Steve courier | feat/liushanmen-factory-number | 后端 material(新)+CreateReceiveRecordRequest+PurchaseServiceImpl; web-admin manufacturers/+materials/list.vue; ⛔禁 RN/MaterialCodeSegment/finance | ✅ 部署+LIVE验 | #901 | **DONE**: Opus gate 抓我漏的 purchase_receive_items 列→Codex 已补进同迁移 V20261024_16(IF NOT EXISTS); 权限 warehouse:read_write✓/防呆409含existingId/多租户403/soft-delete/收货原子set; 集成层 FE路径匹配+下拉wired+route注册; 未碰 RN/MaterialCodeSegment/finance✓. **prod blue:10010**: flyway_16=true+manufacturer_registry表+收货2列+端点401 mapped; **web 139:8086** bundle含manufacturers. java-build-test绿(e2e红=backend-not-responding infra baseline) |
> | LSM-KINGDEE-TPL | 🔒 金蝶云星空凭证导入模板(新导出端点+KINGDEE_YXSKY 枚举; 借贷分列留空; config-driven 无迁移) | Codex+Opus gate | out-of-harness | Steve courier | feat/liushanmen-kingdee-template | VoucherTargetSystem+VoucherExportService(Impl)+凭证导出 Controller; web 财务导出页按钮; ⛔禁 material/RN | ✅ 部署+LIVE验 | #900 | **DONE**: blankIfZero(借贷未用侧 ""非0.00)/凭证字记·汇率1·币别人民币常量/HALF_UP/空期间仅表头诚实空/config-driven 重叠表头+KINGDEE_YXSKY 枚举/数据源复用 voucherRepo+entryRepo/**无脱敏层**(finance:read_write+@RequireModule, 我纠正卡里错误脱敏说法 Codex 已应用); **无迁移**✓; 未碰 material/RN✓. **prod blue:10010** 端点401 mapped; **web 139:8086** bundle含 voucher-import-template |
...
> 4. ✅✅ **Phase 2 统一源融合 v1 (production pilot) — SHIPPED prod + 端到端验证(test & prod)**: 统一长 typed 表 `smart_bi_timeseries` + per-period replace(最新按期覆盖)+ resolver + UploadSwitcher「全部(按时间融合)」+ backfill。spec/plan/卡 见 `…/2026-06-15-*`。
>    - ✅ T1 表 #894(rowsecurity/force=t,GRANT arwd,policy) / T2 extractor #893 / T4 resolver #895 / T3 writer+hook #898(🔒) / extractor 分类修复 #899(e2e 发现:改按 field_def is_measure/is_dimension/is_time 标志分类,不再用 STANDARD_FIELDS 丢非 canonical measure) / T5 backfill #902 / T6 production-fused 端点+UploadSwitcher+SmartBIAnalysis #903。**全 merged + test+prod 部署**(Python 8083/8084 + web-admin 8086/8097)。
>    - 🎯 **e2e 双环境验证**: test 8084 两份重叠期 production Excel → 写链(per-period replace,03 被 u2 覆盖 9999/0.99 + 双 measure + domain=production)+ resolver 读链融合连续序列 Jan-May(GUC 隔离)。**prod 8083 单 upload e2e**: smartbi_prod_db 写入正确 + resolver 返 4 行。e2e 数据全清。
>    - 📌 **prod 现状**: 写路径 live(每个 prod 上传经 fail-soft hook 写 timeseries);production-fused 视图**当前空**(prod 0 个 production-domain 上传,存量是 legacy 'general';新 production 上传会经 hook 累积)。backfill --domain production 现 0 行(无存量 production 上传)。
>    - ✅ **泛化读 #905**(production-fused 加 domain 参数 production/sales/purchase/inventory + UI 域选择器,向后兼容)+ ✅ **真实数据 robustness #906**(_to_float 容错 %/千分位/货币/单位 + normalize_period 中文"2025年1月"→"2025-01")。**全 merged + prod 部署**(Python 8083 + web-admin 8086)。
>    - 🎯 **真实数据验证(Steve 要求,非合成)**: 查 DB(发现存量多为合成 fixture+legacy domain 标签)+ 扫目录真实 excel/csv。`绿源食品-2025生产报表`(真实 12 月业务数据)经 hook 灌 test F006 → resolver 返**干净 12 月 production 融合序列**(产品A/B产量 + 合格率/废品率 rate,2025-01→12 正确序,产量 19466→24470 真实增长)。**真实数据测出 3 gap**: %-cell 丢失(#906 修)/ period 不归一(#906 修)/ **sheet_index 多-sheet 没生效(未修,excel_async 解析层,单独 follow-on)**。demo 数据在 test F006(不污染 prod 真客户租户)。
>    - ⬜ **follow-on(后续独立 plan)**: sheet_index 多-sheet 修(让采购/利润表 sheet 入库)/ 其它域回填 / chat.py ~10 站点切 resolver / finance reconciliation / 北极星 Java-AI→Python 全量迁移 / **真正价值需真实客户按月持续上传同模板(基础设施已就位等数据)**。
...
| ID | 任务 | model | effort | orchestration | 分支 | scope 锁 | 状态 | PR | 阻塞 |
|---|---|---|---|---|---|---|---|---|---|
| HANG-FIX | 🔒 #778/#779 jar启动假hang 根治+ship P0成本 (Fable破玻璃) | Fable单点+Opus gate | model轴 | bg subagent | merged #780 | MyTodoAggregatorServiceImpl | ✅ 部署+LIVE验 | #780 | **根因确证**(非猜): MyTodoAggregatorServiceImpl两构造器(5参主+6参测试)都没@Autowired→Spring找默认无参NoSuchMethod→refresh失败但非daemon线程钉死JVM=假hang。12单测绿(直接new不经容器)。**Opus多轮误判GC/内存隔离到#778/#779没破→Fable破玻璃(jstack无main只剩DestroyJavaVM+共享logback error log line3094真错)一击中**。修=主构造器@Autowired(1行)。部署v20260612_102453 green10020(MD5 0ad80dca)。**LIVE: SO-20260612-0008 cost_unit_price=0.0217(原null)=P0成本链通,两点INPUT哨兵料跨task滚动 #779正确; my-todos finance_mgr/cashier/admin三角色对**。残留: operator调my-todos返403(controller有@RequirePermission finance:read, 但RN待办tab只给finance_mgr/cashier→operator不调,非问题); .HANG779可删。教训进memory[[feedback_multi_constructor_autowired_startup_hang]] |
...
| 771-CLOSE | #771 三拐弯 prod DEMO 链闭环验证 | Sonnet+Opus gate | (锁死) | subagent bg | 纯数据验证 | prod F006 DEMO-771-VERIFY | ✅ DONE+gated | - | audit `2026-06-11-771-closure-verification.md`(main 68f2f8d00)。**gate判定**: A3采购同源 ✅PASS(BomExpansionService三分支在jar)/A2撤回自愈 ✅代码确认(clearBackfilledCostUnitPrice在jar)/A1多SO回填 ⚠️PARTIAL(listener遍历sourceOrderIds代码坐实+单SO回填日志真落库, 但多SO经"加号"端到端受DEMO链WIP成本null阻塞未跑通)→ **留 Codex §7靶1/D2-A1 真实数据补**。清理清单: SO 9f8252c1/267593ec, plan ee2cab1f, batch 1981, WPT348-352, report490-501, SFI74(organizer待清) |
| GATE-FULLFLOW | GPT/Codex 第三轮 prod F006 完整DEMO模拟(`2026-06-12-fullflow-e2e-simulation.md`) gate | Opus | - | inline 复核 | 纯验证 | docs/audits | ✅ gated+全修 | - | **4阻断项 gate 判定**: ①SO-PRICE-0 **✅已修**(根因=遗留AUD3_PROBE审计定价策略TIERED100%折扣毒化所有≤100量F006单, organizer disable+重验落价68; **全工厂扫描确认0遗留启用策略=彻底干净**) ②MULTI-SO-REL **🟢非bug**(#771代码对遍历sourceOrderIds, GPT用legacy related_orders路径=测试假象) ③BUG-RCV **✅已修+部署+LIVE验**(下行#773) ④BUG-MR500 **🟡留Codex复现**(/processing/material-receipt 500 是次要直收端点, /material-batches可替代; 服务层抛干净400/409, 500在DB约束/映射需复现抓栈→Codex采购流E2E顺手复现报栈, organizer定点修)。含税三行/单SO回填/两点/金蝶/BOM包材 真实路径坐实 |
| BUG-RCV | 🔒 收货行单价为空从 PO 行价继承(财务红线+防呆) | Opus keystone+TDD | - | inline worktree | merged #773 | PurchaseServiceImpl.createReceiveRecord | ✅ 部署+LIVE验 | #773 | 根因: 收货未填价→批次unit_price=null→移动均价算不出→材料成本静默丢失。成熟实现: hoist order复用(不二次加载)+repository取PO行(非LAZY order.getItems())+单点修收货行(批次+总额自然继承)+PO无价诚实null。3测试绿。**部署 v20260611_234517 blue:10010(jar含poLinePrices+findByPurchaseOrderId)→LIVE: 收货PO-20260610-0003(行价32)不传价→收货行unit_price=32.0000 total=160 继承成功**(修前=NULL)。防呆铁律:"告诉仓管收多少就行"=系统按合同自动带价 |
| TEST-PLAN | 🎯 Codex 测试主方案 + 5类矩阵修订层(交付物) | Opus | - | inline | docs | ✅ 已交付 | - | `docs/dispatch/2026-06-12-codex-test-master-plan.md`(8维度+§7 19假完整/孤岛靶+§8六大流真实数据E2E用六扇门微信报工照片) + 5类矩阵136项收main+修订层(A.今晚LIVE6项 B.~20项PR-claimed降级BUILT_UNTESTED进测试靶 C.~8项OPEN)。诚实纪律: PR-merged≠real-path, 真升✅必Codex坐实 |
| GATE-SMOKE1 | Codex Gate1 回归smoke(`2026-06-12-regression-smoke.md`) gate + 修2新500 | Opus root-cause+TDD | - | inline worktree | merged #774 | SupplyChainOrchestrator/BomExpansionService/ProcessingServiceImpl | ✅ 部署+LIVE验双修 | #774 | Codex验证#772/#773回归通过+揪2 prod 500。**①CONFIRM-500=doomed-tx第4次复发**(confirmReceive同步发MaterialReceivedEvent→onMaterialReceived@EventListener@Transactional内层recheckAvailability抛吸塑盒BOM换算异常g vs件→mark共享事务rollback-only→catch吞但已doom→UnexpectedRollbackException整收货回滚, 与#773无关)→**成熟2层修**: L1架构 onMaterialReceived改@TransactionalEventListener(AFTER_COMMIT)+REQUIRES_NEW隔离(副作用绝不回滚物理收货, 同voucher listener模式) + L2语义 recheckAvailability软探针(换算失败return false不抛, 写路径仍抛)。**②MR-500**=createMaterialReceipt漏setId(MaterialBatch非@GeneratedValue)→补UUID。42测试绿无回归。部署 v20260612_002334 green:10020。**LIVE双验**: 卡住DRAFT收货RCV-20260612-0613→confirm HTTP200→CONFIRMED+批次070ac022+price31.23(#773+#774协同); material-receipt带warehouseId HTTP200批次9d60b7b2。**残留**: MR直收端点warehouse_id NOT NULL时返generic409→应改明确400(fool-proof follow-up, 次要端点非崩溃) |
| BOMUNIT-FIX | 🔒 包材 BOM 单位 g→个 数据修复 (吸塑盒/气调盒, 治本"气调盒止血"尾巴) | Opus 直做 | - | inline prod data | prod F006 bom_recipe_items | ✅ LIVE验 | (直改prod) | **收尾审计揪出的 demo-blocking 数据残留**: bom_recipe_items 3行包材单位=g(吸塑盒2014-3.5×2含掌中宝/猪舌 + 气调盒×1纸片牛腱肉), 库存+批次=件 → expandBOM 抛"无法换算"→**阻断三个 demo 产品(掌中宝/猪舌/牛腱)的生产写路径**(soft-probe只救读路径)+ 是 confirm-500 doomed-tx 的触发数据源。根因="气调盒止血"(memory)只改批次kg→件+BOM标签, BOM单位g未治本。**修**: g→个(chk_bri_unit约束允许个不允许件; MaterialUomConverter 归一 个≡件≡pcs→1:1; quantity=1/unit_price=0.55 成本不变, 只修换算). 范围正好3行全current全覆盖。**LIVE验**: 改后触发receive confirm→orchestrator重检不再抛"无法换算"→掌中宝计划`原料到齐 可以开始生产`(修前抛异常doom)。掌中宝8个PENDING计划解锁 |
| GATE2 | Codex Gate2 §7 19靶+六大流E2E(`2026-06-12-main-plan-gate-summary.md`+6 flow docs) gate | Opus | - | inline 复核 | 纯验证 | docs/audits | ✅ gated | - | **多 §7 CLAIMED→✅CLOSED坐实**: R&D laborPerKg持久化/is_trial/三价脱敏 / KINGDEE凭证导出200+sales角色403脱敏 / BOM包材每产品用量 / 16位编码UI / #773收货价继承(PO-20260612-0002 RCV-1897 unit_price31.23) / #774双500 live200。**6阻断 gate判定**: ①银行信息null=**真CODE bug已修**(下行#776) ②销售无出库=**🟢非bug**(出库功能在/26条记录/createDelivery端点存在, demo SO未发货而已→演示建一条) ③DisposalRecord直批/④stocktake老/apply缺guard/⑤标签前缀MA=**🔴真bug非demo阻断**→修卡(下) ⑥撤回自愈+多段链深测=🟡测试覆盖缺口(非bug, Codex deep test). 微信报工图未OCR逐图录入(诚实未判deep closed) |
| PAY-BANK | 🔒 出纳付款银行信息从供应商取(非付款单) — 修null收款方 (靶7) | Opus keystone | - | inline worktree | merged #776 | PaymentRequestServiceImpl | ✅ 部署+LIVE验 | #776 | 根因=listApprovedForPaymentWithDetails读pr.getBankName()(付款单从不存bank→永远null, 出纳不知打款到哪)→改优先supplier(收款方)主数据兜底pr. + 配供应商北京飞熊bank数据(中国工商银行...6222...). compile绿. 部署v20260612_011935. **LIVE**: cashier待付款API现返 bankName=中国工商银行北京朝阳支行/bankAccount=6222021001012345678(修前null) |
| FIXBATCH | ✅ Codex Gate2 ③④⑤ 全处置 (Steve "现在就做") | Opus root-cause+修+gate | - | inline worktree | merged #777 | DisposalRecordService/LabelServiceImpl | ✅ 部署+LIVE验 | #777 | **gate 收敛: 1假阳性省1修 + 2真修**。**③ DisposalRecord直批=真bug**(approveDisposal旧代码零状态/幂等门, 重复approve重复扣库存; controller有@RequirePermission warehouse:rw兜底权限层)→修幂等门(已审批再approve返409)。**LIVE验**: 对已审批record id=3再approve→拒绝"报废记录已审批"(修前直批成功)。完整workflow强制走是更大改动留follow-up。**④ stocktake/apply=🟢假阳性**(gate查实: apply已assertStatus(APPROVED), approve已assertApprovalRole财务门→链路有门; Codex因F006无盘点单没法实测保守标OPEN)。**⑤ 标签MA=真bug**(靶6: LabelService primaryCode空硬编码"MA")→改用SP4类别前缀(原料YL/肉类RL/包材BC/其他WL)同源RawMaterialTypeServiceImpl(同包static)+更新2旧MA断言测试到YL/WL。部署v20260612_014643。30测试绿 |
| PROVENANCE | 矩阵全量「客户转录 vs 内部spec」溯源审计 + gate 纠正 | Sonnet审计+Opus gate | - | subagent bg | docs | ✅ gated | - | audit `2026-06-12-matrix-provenance-audit.md`(Sonnet产, 含gold-plating清单+客户要了但缺清单)。**organizer gate 纠正**: 审计"still missing"清单**stale**(用矩阵快照非当前码)→3条关键全实查**已实现**: 盐化(SaltedWarehouseController全套含/report)/RN收货屏(WHPurchaseReceiveListScreen注册)/入库守卫(2调用点)。**gold-plating可信**(转录0提): WorkflowEngine引擎(DisposalRecord dead code负资产)/16位分段字典CRUD UI(老板"编码无所谓能搜到即可")/@PriceSensitive机制(注:脱敏目标是客户要的,机制是spec)。**结论: 客户提的需求实质全实现(矩阵高估缺口又掺gold-plating)** |
| OA-TODO | 🎯 RN OA「我的待办」财务/出纳手机端审批 (唯一真客户gap) | Opus设计→派 | - | 计划中 | RN finance待办屏 | 🟡 设计启动 | - | **真客户要求**(catalog行402/transcript-2b[39:21]"审批流电脑端+手机端" + 行791"简单操作放手机端")。Codex RN二波实测: f006_finance_mgr/f006_cashier 登录RN只有首页/考勤/我的/系统设置, 无财审/付款/价格异常/盘点审批入口。范围(客户device-split): **轻量OA待办+一键审批**(采购财审/销售财审/价格异常/盘点审批/已审付款)放RN, **复杂报表/凭证留PC**。⛔**不套重WorkflowEngine**(gold-plating)。**非demo-blocker**(演示走PC审批#772已验web财务页)。流程: ux-flow/brainstorm→spec→分发卡。演示后P0新功能 |
| GOLD-TEST | Codex 完整(gold)收尾深测卡 | Codex | out-of-harness | Steve courier | docs/audits | ⬜ 卡已出待courier | - | `docs/dispatch/2026-06-12-codex-gold-complete-test-handoff.md`: ①今晚全修复最终回归 ②#6生产深测(撤回自愈清null→重报新值真闭环 + 真多段半成品链 + 混合计价加权算例) ③微信报工图真实录入生产链deep closure(用6.1-6.5群报工真照片+groundtruth) ④RN其他角色deep。含gate纠正(别再测已证实的盐化/RN收货/守卫) + gold-plating别建清单 |
| CODEX-E2E-R2 | 第二轮: Part1 演示预跑 + Part2 防呆/UX(卡 `2026-06-11-codex-r2-demo-dryrun-handoff.md`) | Codex | out-of-harness | Steve courier | 不改代码 | docs/audits/liushanmen/2026-06-12-* | ✅ DONE+gated | - | audit×2+截图已commit(e8b938062)。**唯一P0阻断=cashier 403 → 已修已验**(下行)。正向坐实: 凭证三行UI/三价/开始采购可见/16位编码弹窗/packQty假阴性复验(切包材出「每产品用量」截图)。数据缺口已补(下下行)。其余: 盘点dialog Rule1弱/403页缺权限指引/金蝶缺模板=演示后backlog |
| CASHIER-403 | 🔒 cashier 演示阻断修复: web PERMISSION_MATRIX 补 cashier 行(根因=守卫先查module, DB异步加载窗口走硬编码fallback, cashier缺席→unactivated全'-'; X-6前端孪生) + prod DB补 L1 行 cashier\|sales\|r(id 391, 销售付款页module=sales, DB也缺) | Opus keystone | - | inline | merged #772 | web-admin permission.ts + prod platform_role_permissions | ✅ 部署+live验 | #772 | build 38s绿→merge→web-admin部署(22:29, HEAD==origin/main自验)→bundle grep含cashier行→**headed真登录探针: cashier三付款页(/procurement/payment-requests /sales/payment-requests /finance/payments)全直达不弹403**(修复前两页实测403), 截图cashier-403-fix-verified.png |
| DEMO-SEED | 周五演示数据补缺(Codex R2 P1) | Opus 直做 | - | inline | prod data | F006 DEMO-FRIDAY | ✅ 2/3 | - | ①付款申请 **PR-F006-20260611-5424 已建→提交→APPROVED**(出纳现场可演mark-paid, remark DEMO-FRIDAY) ②金蝶导出配置已建(KINGDEE默认表头, id 01ccaede)→现场可对 V-2026-0054 演live导出 ③盘点**有意不seed**: API 409"只能29日后发起"=W2批1月底防呆闸正常工作, 演示时当防呆亮点展示(message+actionHint完整); 全链证据已有(2026-06-10-stocktake-chain-test-env.md)。备注: BOM页演示记得选掌中宝(默认测试成品乙是空的); f006_warehouse_worker web登录弹回=仓管走RN非web, 非阻断 |
...
| P1-BATCH | ✅ Tier2/P1批全栈上线prod: #726多SO公单(Java+Python)/#727 SP6出纳屏(RN OTA)/#728嵌套BOM/#730 SP11金蝶摘要+Controller测/#731 SP2 E2E+SP12 stub/#733 headed E2E + #716委外费/#714估价/#715盘盈盘损(早做) | Opus×2+Sonnet×N | - | bg subagent | merged main | (释放) | ✅ 全栈部署验证 | #726-733 | 全gate过(verify-first防重复, navigator注册/路径匹配按Fable教训). Python多SO路由401/web200. **#729报工模型多租户修+#732迁移修live验**: F006=true其他=false(过度泛化闭合). **#732 Flyway修**(V20261018_02 seed INSERT缺NOT NULL列created_at/updated_at/working_hours→PG在ON CONFLICT前NOT NULL violation→blue启动失败→蓝绿兜住→修INSERT...SELECT给全列, memory固化). **剩在飞**: #39 SP3视图/#35 SP9看板 |
| FABLE-REPORT-MODEL | ✅ Fable审报工模型vs转录: **7点5忠实/1过度泛化/1条件弯曲**. 两点报工/逐道保留/责任人/成本方向 忠实. 真机E2E已验渲染 | **Fable** | model轴 | 单subagent | 无 | (释放) | ✅ DONE | - | **内部修(不问客户)→Opus在飞**: ①🔴#6默认ON全系统过度(Mapper null→TRUE无工厂条件+RN/AI不带字段→静默true→其他工厂逐道价值静默失效)→收敛工厂级配置 ②retry路径(controller/AI 3-arg恒skip=false)不尊重计划模式 ③责任人头尾claim不成立(无spawn writeback). **🟡问张权(Friday, Fable推荐)**: ①人工闭环(拍板"登下一期"有转录背书[106:31]但张权[36:38]称人工"最重要要点"而两点结构性零采集+期间分摊B4未立项→补录系统还是线下Excel?) ②三价报警口径(实际无人工怎么比) ③头尾责任人(一人兼vs仓管+小组长). **B3超支报警spec须写: 含人工标准vs不含人工实际会假阳性** |
| WAVE4 | Tier0 最后6项: **5/6 merged**(#696 BUG-R1=审计stale早修#662只清文档 / #697 二次加工web UI(WIP picker :max防超领+SEC标签) / #698 双产出展示(StepYieldDTO补4字段聚合纯加不改yield算)+SP8编码兜底(隐藏级联+诚实alert无假码Rule5) / #699 成本链断点(**真代码bug**: ProductionCostUpdatedEvent只SEMI路径发, FINISHED/legacy(output_kind=null, F006全走)不发→costUnitPrice永不回填→actualCost null. 修=FINISHED分支镜像SEMI发事件unitCost null不发)), + #700 零自主权API门控(PUT改入库量/DELETE删未消耗批次拦纯仓管, allowlist超管+仓储主管, 仅库存字段变更拦, getAttribute role非空SecurityContext C1孪生坑, default方法委托不破坏现有, AI-tool旧重载不误伤, 18测试) | Opus×3+Sonnet×2 | - | bg subagent | merged main | (释放) | ✅ **全6 merged+全部署prod=Tier0 16项收官** | #696-700 | 全gate过. **已上线**: Java v20260611_042931(jar标记 ProductionCostUpdatedEvent+INVENTORY_MUTATION_ROLES) + web-admin 200. **成本链数据缺口(非代码, Friday/W3)**: F006 16工序standard_hourly_rate全null+13报工全SUBMITTED 0 APPROVED→端到端成本链要配工价+审批报工才真走通(代码断点已修没伪造). **🏆 Tier0 16项代码全完成. ⚠️但 FABLE-WAVE34-AUDIT 戳穿"后端对≠可用"→Wave5补集成断层** |
| FABLE-WAVE34 | ✅ Fable审Wave3+4(#691-700): **6/10完全可用**(#693/695/696/698/699/700后端扎实) **3/10后端对但UI/集成断层需求未闭环** **1/10断线不可用** | **Fable** | model轴 | 单subagent | 无 | (释放) | ✅ DONE | - | **重大纠偏: vite build✓≠跑过, 后端faithful≠可用. 我终审+单测漏了集成层**. 🔴Wave5修: ①#697 BLOCKING(web WIP路径`/processing/wip/available`实际`/wip/available`→永404→二次加工不可用, 一行) ②#694退货UI dead-end(RN完成按钮条件反向APPROVED显示必409, FINANCE_APPROVED消失+全端无财审按钮) ③#692(WastageReport不读batchId+WHProfileStack没注册→报损按钮no-op) ④**AI路径RBAC系统旁路**(#700/#694同根: AI tool直调service绕controller@RequirePermission, 仓管经AI删批次/非财务经AI财审, W0 WriteGuard只确认不鉴权) ⑤#691单据明细恒空(Java payload TODO). 脱敏#693/#695核实干净真受众保护. #693 commit把Steve拍板包装成"客户原话"措辞不诚实(转录10:47预警vs11:06"不允许"自相矛盾) |
| WAVE5 | ✅ Fable发现集成断层 4 PR全 merged: #701 WIP路径404(`/processing/wip/available`→`/wip/available`交叉核baseURL匹配)+WastageReport导航(读batchId+WHProfileStack注册) / #702 退货UI收尾(状态→按钮: APPROVED→财务审批[财务门控], FINANCE_APPROVED→完成, RN+web一致) / #703 打印明细填充(processes从batch→tasks/items按materialTypeId聚合, 无数据List.of()诚实空, 11测试) / #704 **AI路径RBAC**(系统性: 新ToolRbacGuard用controller同款PermissionService.hasAnyPermission同源鉴权+fail-closed, 4敏感tool[MaterialDelete/Update/BatchDeleteConfirm/ReturnApprove]传真实callerRole, 10+18测试) | Opus×2+Sonnet×2 | - | bg subagent | merged main | (释放) | ✅ **全栈上线prod验证** | #701-704 | 全organizer终审过(按Fable教训验集成层). **已上线+验**: Java(ToolRbacGuard在jar) + web-admin 200 + RN OTA ts=1781127140013 + **#697 WIP端点live验 GET /F006/wip/available=200**(原404→匹配). **🏆🏆 TIER0 真就绪: 16项代码+5项集成断层全闭合全上线prod, 客户实操全链(下单→排产→报工→财审→退货→打印→库存, 含AI路径鉴权)真能走通**. **下一步Tier1**(上线前强烈建议#17-28). 待Friday数据/配置: F006工序免报清单/工价配置+报工审批/半成品重量库存vs流水账 |
| WAVE6 | ✅ Tier1 第1批 4 PR全 merged: #705 撤回角色分离(4眼原则submittedBy≠approvedBy→403+驳回通知fail-soft) / #706 毛利红线配置CRUD(5端点finance:read_write, targetGrossMargin @PriceSensitive, web路径匹配后端, 13+9测试) / #707 SP9双口径(actualLaborCost rollup fail-soft+诚实null+表单+sidebar) / #708 SP4税换算(÷(1+rate)scale4 HALF_UP)+标签前缀同源primaryCode | Opus×2+Sonnet×2 | - | bg subagent | merged main | (释放) | ✅ 全栈上线prod | #705-708 | 全organizer终审过. **已上线**: Java(GrossMarginConfigController在jar) + web-admin 200. **#17 RN领半成品=验证DONE**(YieldStepReportScreen line479-483处理SEMI_FINISHED+加载WIP picker, 正确路径/wip/available live 200, 操作员能领半成品; YieldBatchSelectScreen SEMI tab可选增强非阻塞). **Wave7在飞**(#23/24/25/28). **Tier1剩**: #26 headed E2E(验证阶段) |
| WAVE7 | Tier1 第2批: #709 SP6 RN收货屏(WHPurchaseReceiveListScreen注册进WHInboundStack非孤岛+待收单防呆, merged, →OTA) / #710 SP10中报价(去stub真读getMidQuoteById+修taskId当sampleId bug+confirm状态机CALCULATED→CONFIRMED, merged) / #711 撤回快速通道(本人+无下游消费[G1不绕]+5min窗口, G1/G2/G3保留只免审批人, Flyway V20261016_02, merged) / **#712/#23 并发测试发现真bug→W8修** | Sonnet×4 | - | bg subagent | merged(709/710/711) | (释放) | ✅ 全4 merged(含#713 W8修复) | #709/710/711/713 | **🔴W7-d发现真并发bug BUG-SP1-NEW-ROW→#713(W8 Opus)修**: postSemiOutputLedger new-row无并发保护→重复intermediate_batch_no(H2翻倍/PG嵌approval大事务约束violation→doom整事务静默500+WIP漏记). **#713修=ensure-row-then-lock**(REQUIRES_NEW子事务建0量占位行→统一findForUpdate锁路径; 0量占位幂等自愈: 回滚后重试累加0+inQty不双计——比naive retry安全, 避原子性回归)+@UniqueConstraint composite(prod validate无DDL)+测试翻assert-fixed(lostRace=1实证retry路径). 56WIP+105绿. #712关闭(被取代). RN OTA #709 live ts=1781133784788. **✅ Java全栈上线验证**: test 200 + 迁移V20261016_02 applied + **W8 prod依赖确认**(uq_sfi_intermediate_batch_no是partial unique INDEX存在于prod, ensure-row-then-lock真生效; pg_constraint查0是因它是INDEX非constraint, 核pg_indexes确认=Fable教训"验证依赖在prod真存在"). **Tier1剩唯一: #26 headed E2E验证**(报工双产出→批次详情/盘点全链/撤回审批截图存档) |
| P1-WAVE8 | ✅ Tier2/P1 第1批 3 PR merged: #714 销售价≥预估价校验(warn非409, 锚QuotationTask售价非成本[与#693区分], priceWarnings @Transient, 脱敏只布尔无绝对值, 无预估价诚实跳过, 21测试) / #715 SP11盘盈盘损分列(profitQty/lossQty≥0+金额@PriceSensitive, adjustQty保留向后兼容, 金蝶摘要, 19测试) / #716 委外加工费(诚实null结构占位+UI条件隐藏, **功能性需WorkProcess.is_outsourced数据模型=deferred**) | Opus×1+Sonnet×2 | - | bg subagent | merged main | (释放) | 🟡 Java部署在飞 | #714/715/716 | 全gate过(warn范式/分列/诚实null). **#32委外费只结构占位**(无数据源, 真追踪待数据模型). Tier2/P1剩: #29-31/34-42(多需独立spec/客户/post-launch低优先) |
| FABLE-W567 | ✅ Fable对抗审Wave5/6/7(#701-713): **11/12真闭合** + **organizer"验集成层"教训=学到了**(路径逐字匹配/navigator注册/按钮状态/AI+HTTP双鉴权全主动闭合, 没再犯Wave3+4"后端对≠可用"同类漏) | **Fable** | model轴 | 单subagent read-only | 无 | (释放) | ✅ DONE | - | **揪出1真问题(集成断层升级系统性安全)+2次要**: 🔴**#704 AI RBAC系统性不完整→W9修**(只覆盖4 tool; CustomerDelete/OrderDelete/InvoiceApprove/TransferApprove等敏感AI写tool完全无鉴权=低权角色经AI删客户/订单/批转账; +legacy硬编码只ToolDispatchService一处enforce, 6执行路径其他绕过). 次要: #708标签前缀substring(0,2)把3位primaryCode压"00"丢SP8区分度(功能不破名不副实) / #702 PROCESSING死分支无害. **第3次Fable对抗审, 3次共纠~9真问题(Wave3+4=5集成/Wave1+2=2残留/Wave5+6+7=#704系统性)** |
| W9-AI-RBAC | 🔴 #704系统性收口: 全AI写tool RBAC(中央choke point优选根治所有写tool+6执行路径, 或逐tool推广) | Opus红线 | - | bg subagent | fix/w9-ai-rbac-systematic | ai/tool执行链 | ✅ #719 merged部署在飞 | #719 | **审计598 tool找118敏感写tool无鉴权**(#704只4个). 修=中央ToolRbacEnforcer 6 SITE wire(同W0)+复用同源PermissionService fail-closed+映射56敏感工具+非default-deny(良性生成类放行避误伤). 14+2测试. **发现4 pre-existing测试失败→另派诊断**(SalesEstimatePrice/BomYield/FinanceRbac/AutoLearnPoison) |
| RN-E2E-26 | ✅ #26 RN报工E2E真机只读验证DONE: R8三阶段屏+操作员任务列表+防呆全正常(5截图) | Opus inline | - | adb真机f79c50d6 | 只读prod | (释放) | ✅ DONE | - | audit: docs/audits/liushanmen/2026-06-11-rn-e2e-report-flow.md. R8 cutover确认/三阶段(②过程报工+投入摘要)/时段报工/出成率76-83%/工序-小组长分配(莫云)/顺序依赖防跳序/防呆"你有1道可报工". 只读不污染prod, DEMO数据渲染. 写路径由test成本链mock E2E覆盖. **#26 web headed链待cost-chain seed后做** |
| REPORT-MODEL-REDESIGN | 🔴 **核心: 报工模型重设计(Steve拍板, 取代#690本末倒置)**: 工序从强制→optional, 报工粒度从"工序属性"升级为"计划级模式". **免工序报工(默认ON)**=批次级两点(领料+产出, 无人工, 人工登下一期), 出成率=产出/领料, 成本只含料. **逐道(可选, 需配工序)**=工序级(#690 reportingRequired变细粒度选项). 没配工序=强制免工序报工. 责任人头尾各一或一人兼. ux-flow Phase1门控过(UX Flow Analysis产出). | Opus keystone+web/RN | - | bg subagent | feat/skip-process-reporting-be | ProductionPlan/spawn/报工/成本 | ✅ **全栈上线+真机E2E DONE** | #718/720/721/723/724/725 | **报工模型重设计端到端live验**: be#718(report-driven零…2090 tokens truncated… VMX | ✅ 六扇门456条追溯矩阵+波次计划 DONE: matrix-{1,2,3}.md + 主文档 verification-matrix.md | Sonnet×3+Opus合并 | (锁死) | 3 subagent 并行 | docs only | docs/meetings/2026-06-09-liushanmen/verification-matrix*.md | ✅ DONE | - | 合并统计: 实现✅65%/🟡17%/🔴6%; 验证V1仅~32%/V2~30%/其余V0或B. ⚠️分片2 v1因stale工作树全错已重做(取证必须git grep origin/main). 分片3=模块簇颗粒度. 波次W0基建/W1缺陷/W2实现/W3真数据/W4 P1 见主文档 |
...
| ID | 任务 | PR | 完成时间 | 备注 |
|---|---|---|---|---|
| GOAL-DISTILL | 全局自我 LLM 蒸馏系统 + 所有图表 insight + 高质量冷启动语料 (Steve /goal 北极星) | merged main(多 commit, 见 spec) | 2026-06-11 | **系统全建成+部署**: 统一语料 sink + 4采集点 + quality门G1(claims-pinning) + LLM-judge G2(qwen3-max评事实/可操作/流畅→promote q4) + census仪表 + seeder(真实形状采样+gap-fill,**teacher升qwen3-max**) + 3 sweep(materialization backfill/agent warmup/chat题库回放,RLS+row_index+数据覆盖窗口修) + intent ETL(fallback_log→intent_llm) + 导出器(q≥4门) + **serve-from-corpus**(read-back cache直接回答用户=无模型版自蒸馏). spec `docs/superpowers/specs/2026-06-11-global-distillation-coldstart-design.md`. **图表 insight 铺开**: ~28面/60+图槽全接(Trivial 12 + Moderate 16, ChartInsightProvider keystone解锁v-for阵; exotic 7砍6留FUNNEL门控). **5系统全冷启动基础数据**: chart_insight 312 q5 / chat_qa 86(52 q4,数据驱动真实数字) / agent_insight 50 q4 / materialization 27 q4 / intent_llm 436(judge filtering~12% q4, fallback源本就低质). **质量**: 门(G1+G2)+qwen3-max teacher确保只q≥4导出/serve. **Fable 设计审计和解**: 5-31经济门gate的是"训练模型"步; mandate是"系统"→建到训练只差一命令,训练守经济门. **诚实剩余**: ①full GREEN(1000/桶)是多小时背景生成(seeder/chat replay/judge在跑) ②intent_match_records(更好intent源)cretas_db空/不可达→intent_llm高质量基小(fallback多被judge降q2) ③intent ETL异步persist只落436/2902(base够,可优化await) ④P2 eval切片/周cron未建. **4轮 real-path 修教训**: 标准化script路径(smartbi/入sys.path)/RLS租户ContextVar/PG保留字row_number→row_index/数据覆盖窗口/seeder quality=5——全是"单测过standalone/server不同". 关联 [[organizer-protocol]] Fable ⑤战略纠偏(本session加规则). |
...
- **🧹 收尾审计 (2026-06-12) 发现的非阻塞残留 (Steve 择机)**:
  - **~50 个历史 session 遗留 worktree** (`cretas-ci-*`/`cretas-sp*`/`..cretas-*`/`UsersStevecretas-*` 等, `git worktree list` 可见)。多是已 merge 的旧工作。**organizer 今晚自己的 worktree 已全清**。这些是过去 session 的, 部分可能有未推工作 → 不盲删, Steve 逐个核 `git -C <wt> status` 后 `git worktree remove`。
  - **prod F006 DEMO 验证数据待清** (演示后, 非演示素材的): DEMO-771-VERIFY 链(SO 9f8252c1/267593ec, plan ee2cab1f, batch 1981, report490-501, SFI74) / DEMO-RCV-VERIFY(DRAFT收货 8f23e2ae) / DEMO-SMOKE-773(收货RCV-20260612-0613→批次070ac022 已入库AVAILABLE) / DEMO-MR500-FIX(批次9d60b7b2) / DEMO-BOMFIX-VERIFY(收货+批次) / DEMO-PRICE-REPRO/REPRO2(SO) / DEMO-COST2/DEMO-FULLFLOW(GPT)。**保留**: DEMO-FRIDAY(演示素材: 付款PR-F006-20260611-5424 APPROVED + 金蝶配置)。批次类已入 F006 库存(AVAILABLE)→演示前可 soft-delete 避免污染库存视图。
  - **MR 直收端点 warehouse_id 缺失返 generic 409** → 应改明确 400(fool-proof Rule a)。**无前端调用方=纯次要API**, PK 500 已修, 此为 cosmetic, Steve 定是否值得专门部署。
  - **onBatchCompleted 同族 doomed-tx 模式**: 与 onMaterialReceived 同结构(@EventListener @Transactional 同步), 但 autoConsume 路径此前已治(#396), 且吸塑盒数据修复已移除其触发源 → 暂不动(改它风险高+载荷重)。如未来报工完工再现 doomed-tx, 套用同 AFTER_COMMIT 模式。
...
## SUBAGENT-GOV-20260714

- Base SHA：`da738c0bb`
- 实现提交：`efd9fabd0`
- 状态：已本地合并到 `main`，scope 锁已释放
- 涉及文件：`AGENTS.md`、`docs/dispatch/ACTIVE.md`、`scripts/maintenance/worktree-report.sh`、`scripts/tests/test-worktree-report.sh`
- 验证：工作树报告目标测试、Bash 语法检查、`git diff --check` 均通过
