...
| ORDINARY-AUDIT-20260716 | `8fcbbde98af4edbec55b064da5d24f651c6ca201` | `/root/verify_material` | 只读复核普通反馈1-8：价格语义、BOM成本单位、BOM展开ID、采购草稿供应商、原料税务口径、物料需求快照和箱/g/kg换算主链均已修复；生产只读请求成功且业务写请求为0。发现询价报价显式0%会被前端吞值及物料需求页保留未调用的逐行计划查询代码。 |
| ORDINARY-ASSURE-20260716 | `c17fcb30a38eab572993d49eaa140c51afbe2e59` | `/root` | 修复询价报价0%保留和物料需求页逐行计划查询旁路；新增租户隔离的生产计划单据追踪，覆盖销售、采购、入库、领料、生产批次、核对结单、成品和销售出库并支持跳转。原料税率测试夹具同步强制L1-L3编码契约；后端50项、Web12项、vue-tsc和Vite生产构建通过；[PR #1389](https://github.com/Stevenjxie/cretas/pull/1389) 已合并。 |
