...
- `ML-LIUSHANMEN-VISION-LAB-20260810-001` — `review` — Owner: `/root` — Base SHA `85cc75e6216100a842ab59cae8c69855346acfa3`；已搭建六膳门本地 YOLO 持续训练闭环：生产已审核照片只读 keyset 增量下载、SHA-256 内容寻址、SQLite watermark/回执、主动学习难例挖掘、任务级 split、人工确认 MARK、RTX 3060 训练、PT/ONNX 判定一致性、27 张保护集完整生产链回放、候选注册、误报/召回/延迟门禁，以及模型备份、健康检查和失败自动回滚。云端 VL 默认关闭且额度上限为 0。真实初始化下载 1,307 张唯一照片（337 个任务、2.884GB），再次运行新增为 0；线上 tray/label SHA、阈值 0.20、VL off、服务 active/health 200 已核。保护集 7 个独立缺陷、20 张正常图，与 240 张训练队列零泄漏；当前生产基线命中 3/7、正常图 26 个误报，自动晋升要求至少 4/7、两张新缺陷 2/2、召回不回退、误报下降至少 5%。现有 240 张队列为 0/240 reviewed，`D:\CretasVisionLab\attention\MARK-NEEDS-ANNOTATION.txt` 已生成，故新候选训练按设计暂停、生产模型未变。Python 单测 7/7、Ruff、py_compile、PowerShell AST、fixture dataset、2 张自动挖样 dry-run、计划任务实跑均通过；新任务次日 10:00 运行且 LastTaskResult=0，两个旧失败任务仅禁用未删除。scope 随本归档释放；PR/CI、合并、实际新候选训练、模型部署和业务验收分别报告。

- `BUG-F006-WORKFLOW-RAW-BINDING-LINE-20260810` — `review` — Owner: `/root` — Base SHA `4d96d7b99ad1377f0cadb962e3e297aef2a38b1f`；按用户生产手测反馈收口单独原料 Cell 的身份语义：已有名称能在本产品 BOM/原料档案中唯一精确匹配时，仅在加载期内存投影补齐原料 ID、编码和单位，不触发 dirty、自动保存或草稿写入；已绑定 Cell 默认只显示同一身份，通过显式“更换原料”才重新打开分类与 SKU 选择，重名/无匹配继续 fail-closed。辅料/包材投影边使用与普通 Workflow 相同的蓝色实线和箭头，仍不可编辑且不进入 Workflow 序列化。目标 Vitest 4 文件 77/77、`vue-tsc -b --force`、Vite production build、`git diff --check` 通过；未改 Java、数据库、权限、发布/启用或 BOM 写 API，生产业务写入为 0，scope 随本归档释放；PR/CI、合并、exact-main Web 部署和线上验收分别报告。

- `BUG-F006-WORKFLOW-BOM-OVERLAY-EDGE-VALIDATION-20260810` — `review` — Owner: `/root` — Base SHA `ea1c4b9ebcb66ee2a2b0d1b6561cf91248a2cd3d`；生产浏览器实操确认辅料与包材 Cell 可拖动且拖动业务写入为 0，但派生连线在 DOM 中为 0 条。根因是 Vue Flow 的 `setEdges` 会复用页面级 `isValidConnection`，无工艺 kind 的 `bom-overlay:*` 节点被普通工艺连线规则静默过滤；本次仅精确放行“辅料 Cell → 所属工序”和“成品 Cell → 所属包材 Cell”的专用节点归属与 handle 组合，其余浮层或工艺连线仍拒绝。Workflow 目标 Vitest 35 文件 389/389、`vue-tsc -b`、`git diff --check` 通过；后续唯一 Web 构建、PR/CI、合并、exact-main Web 发布及浏览器复验分别报告，未改 Java、数据库、权限或非 F006 数据。

- `BUG-F006-WORKFLOW-BOM-OVERLAY-INTERACTION-20260810` — `review` — Owner: `/root` — Base SHA `7c95752b75a5e4f6958811070f8f690a664cc7d3`；辅料/包材 BOM Cell 已改为可拖动的画布布局投影，拖动不产生 Workflow dirty/新版本，BOM 数据仍不可选删；辅料线新增工序顶部专用只读锚点，包材默认间距避开成品 Cell 实际宽度，两类派生线均使用可见 smoothstep 样式并禁止选择、更新和删除。首次点击详情的 ensure-draft 钉住当前 Workflow revision，并直接把返回草稿合入刷新，消除版本列表短暂陈旧导致的“手动刷新后重试”。目标 Vitest 7 文件 117/117、`vue-tsc -b --force`、Vite production build、`git diff --check` 通过；未改后端、权限、数据库或生产业务数据，scope 随本归档释放，PR/CI、合并和 exact-main Web 发布分别报告。
