...
> ## ⚠️ 2026-08-15 晚订正（读本文前必看）
>
> 本文第一节里的两句推断**经 prod 实测证伪**，订正见
> `2026-08-15b-sheet-followup-product-decisions.md`：
>
> 1. 「自动盖章意味着采购单从来不会进入财审模块」—— **错**。模块空与盖章无关：
>    全库采购单只有 **9 张**，且**没有一张**属于配了审批链的那两家工厂。
> 2. 「改状态会把 LIUSHANMEN 的采购收货整条堵死」—— **错**。
>    **LIUSHANMEN 有 0 张采购单**，没有东西可堵。
>
> 更根本的一层：`approval_workflow_instances` **0 行**、`approval_history` **0 行** ——
> 61 条审批链只是**配置**，采购 OA 审批在生产上**零实例**。
>
> **裁定（Steve）**：财务盖章功能**不做**，「就是一个 OA 就够了」。
