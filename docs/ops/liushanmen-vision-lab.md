...
随后只做一次结构表达验证：模型用中心点加 `TL/TR/BR/BL` 四个象限内局部角点固定角点身份，
并保留人工多边形掩码作为训练辅助；推理仍只输出人工语义一致的四点多边形。几何审计确认
96/96 个人工多边形都可由该参数化表达。修正参数头不得全局平均掉位置信息后，容量拟合达到
平均/最差 IoU `0.9902/0.9720`、非法多边形 `0`，但仍有 `3` 个靠近人工边界的台内托盘中心
被判到台外，最差中心准确率 `0.9412`、最差 inside recall `0.9091`，未达到容量门禁。回执
`work-area-roi-constrained-fit-20260812T055838180832Z.json` 的 SHA256 为
`0ddf74b315a59a8b6ee2d3a1768eada42e5f0f00facc5c9b1fd97da573056cb2`。因此不启动 8 折，
不运行完整 7+20，不保存模型、不恢复旧 YOLO、不部署。

对原始库做的只读剩余池审计排除全部既有 photo/task/SHA、旧 MARK、保护集 exact/pHash 与
27 份既有 tray/ROI/label manifest 后，还有 `166` 张照片、`42` 个独立任务（0013 `16`、
0014 `26`）。计划器只在内存中选样，未写 receipt、未建队列；抽检每 SKU 5 个最远 pHash
候选后，画面仍主要是同一工厂、同类金属台车/工作台和相近手持视角，未形成新的相机或台面域。
