...
- **生产只读真值**：F006 共244条未映射物料，244条均能匹配当前启用 L3（230个实际使用 L3）；LIUSHANMEN 共228条未映射物料，仅10条能匹配当前启用 L3，218条没有可证明的当前 L3 映射。两厂业务编码前缀/计数器当前均为空。本次查询仅执行 `SELECT`，业务 mutation=0。
- **安全裁决**：禁止改写16位 `code`、历史外键或分类身份；禁止把 LIUSHANMEN 的218条未匹配行按名称或旧前缀猜测回填。`displayCode` 不是新增数据库列，而是稳定派生值：有 `businessCode` 时显示新码，否则显示旧16位码。
...
- **部署状态**：`NOT_DEPLOYED`；未执行生产回填、未修改 F006/LIUSHANMEN 任何物料或分类。
- **回归状态**：`CODE_AND_TARGET_TEST_COMPLETE`；部署后也必须先分别预览两厂，任何正式历史映射仍需用户单独授权，不能随部署自动执行。
