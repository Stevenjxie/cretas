...
- **回归状态**：`DEPLOYED_NOT_RECOVERED / PENDING_USER_AUTH`；部署后 query-only 证明 `PO-20260721-0001` 仍为 `SUBMITTED`、`workflowInstanceId` 空、`hasInstance=false`。生产业务 mutation=0，未执行恢复、取消、重提、重建或桥接，未触碰 LIUSHANMEN。
