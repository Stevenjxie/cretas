...
- **Commit/PR/main 状态**：PR [#1614](https://github.com/Stevenjxie/cretas/pull/1614) `MERGE_READY`；合入后的 exact main 以本任务最终回执为准。
- **部署状态**：`NOT_DEPLOYED`。
- **回归状态**：`CODE_FIXED_TARGET_VERIFIED`；现有 `SO-20260722-0001` 保留为现场，不重建、不桥接、不修改，部署后须用新单验证统一 OA；未触碰 LIUSHANMEN。
