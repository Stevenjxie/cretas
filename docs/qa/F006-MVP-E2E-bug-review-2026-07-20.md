...
- **范围**：销售订单、采购订单和生产计划三张高频列表；没有扩大修改其他模块。
- **表格对齐**：三张表统一使用固定网格布局，表头、表体和固定操作列共享同一宽度契约；固定操作列增加边界和行背景适配，避免横向滚动时遮住上一列。
...
- **销售订单**：页面主按钮直接打开现有普通新建完整表单，不再经过“选择录入方式”；删除未投入使用的一键快速、二维表格和 BOM 展开前后端共享 UI 类型/组件。弹窗先显示，再并行加载客户、产品、业务员、仓库和客供料候选；单项失败不会阻塞其他候选或产生未处理异常。订单行继续保存和回显权威 `sourceWarehouseCode`。
- **生产计划来源**：新建只保留“存货生产”和“销售订单”。存货生产计划数量选填，以逐道报工和生产小结形成实际产量；销售订单生产数量由订单行带入并只读，不能在计划页改写承诺数量。
- **执行动作**：存货生产可反复执行“生产小结”，正式结束使用“停产”；销售订单生产使用“核对结单”。低频停产/取消仍收进“更多”，高频录入与结算动作保持直接可见。
- **产品与 Workflow**：列表主列只显示计划生产的成品，不再重复输出蓝色关联标签；“工序图”悬浮/键盘聚焦后读取该计划固定的 `selectedWorkflowId + selectedWorkflowVersion`，避免用后来启用的版本冒充历史路线。
