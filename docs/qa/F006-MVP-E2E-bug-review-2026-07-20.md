...
> 本批条目基线：F006 生产现场仅作只读证据，不修改订单、计划、发货单、盘点单、库存或 LIUSHANMEN。代码提交与 main SHA 在整批终验后统一回填；当前统一记为 `CURRENT_BATCH_PENDING_FINAL_SHA`。部署在实际发布前统一记为 `NOT_DEPLOYED`，发布完成后由协调者更新部署指纹和回归结论。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；生产业务 mutation=0。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；未创建第二计划。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；未修改客户/订单。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；当前唯一发货单未变更。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；生产发货单/库存未重放。
...
- **Commit/PR/main 状态**：`NEEDS_BUSINESS_DECISION`；相关通用约束为 `CURRENT_BATCH_PENDING_FINAL_SHA`，无“强制消费0554”代码。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；未重放生产 allocation。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；现有 ST 不变。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；生产 ST/库存未桥接。
...
- **Commit/PR/main 状态**：`WITHDRAWN / BUSINESS_DECISION_REVERSED`；保留两步实现为 `CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；历史记录不伪造。
...
- **Commit/PR/main 状态**：`CURRENT_BATCH_PENDING_FINAL_SHA`。
- **部署状态**：`NOT_DEPLOYED`（实际发布后更新）。
- **回归状态**：`TARGET_TEST_IN_PROGRESS`；只收拢入口，不删除单据功能/责任边界。
