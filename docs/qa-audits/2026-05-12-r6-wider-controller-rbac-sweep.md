...
| 3 | `DictionaryTestController` | `/api/public/dictionary-test` | 4 | ✅ REMOVED | **2026-07-18 correction:** the prior `🟢 SAFE` verdict was wrong. CORS is not server-side authorization and does not constrain non-browser clients; this was a public write endpoint. PR #1432 removed the controller from production source and added an absence regression test. Production deployment is complete; gateway-local `--resolve` verification returned 404, and production access logs from 2026-03-16 through 2026-07-18 contained zero matching requests. |
