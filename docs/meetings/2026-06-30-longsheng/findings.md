...
### 7. 菜单/功能位置调整 (line 43) — 近期项(协作走查,非纯代码)
- 客户原话:"每个大项,比如说采购管理,这里面东西有些功能可能是在别的模块里面,我们俩过一下,然后该调整一下位置,改一下顺序就行了"。
- 性质:菜单 IA 重组——把散在各大项里、归错模块的功能挪到对的模块 + 调顺序。客户说"很简单,改一下顺序"。**需 Steve + 客户一起过一遍**(我们俩过一下),不是后端逻辑,是前端路由/菜单配置 + 顺序。
- 时间序:在**生产环节修完之后、导数据之前**(line 42→43→52)。
- 状态:待 Steve 与客户走查清单后再动(本审计无法独立定"哪个功能该挪哪")。

### 客户时间序列(转录 line 42, 52-53)
1. 先改生产环节(B1 + G1–G4,明后天)→ 2. 菜单位置调整(#7,简单)→ 3. 导数据上线(周五六)。

### 忽略项(客户明确说后面再说)
- 分开录录(每组各自录,line 54-57);NFC / R码 / MC芯片 按员工扫码报工(line 58-59);排班(line 60-62);小组长签到/签退人效(line 64-71)。**本轮不做。**
