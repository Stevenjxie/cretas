...
| A-02 | 研发产品通过后开始建品/建SKU | ProductType + QuotationTask | P0 | ✅已建 | V2 | SP10 QuotationTask.quoteStage 枚举含 FINAL 阶段，quote stage 链路通，无"通过→建SKU"自动触发断言 | headed UI 走研发报价 → FINAL → 新建产品 |
...
| A-24 | 原料/辅料/包材入库时编码全部定死 | MaterialBatch + RawMaterialType | P0 | ✅已建 | V2 | MaterialBatch.rawMaterialType FK 约束存在，链路通，无"入库后禁改编码"强校验测试 | API断言 PUT /material-batches/{id} 改 rawMaterialTypeId 返 4xx |
| A-25 | 新建成品时编码/品名全部定死 | ProductType | P0 | ✅已建 | V2 | ProductType entity 含 code/name 字段，链路通，无"建成品后禁改编码"强校验断言 | API断言 PUT /product-types/{id} 改 code 返 4xx |
...
| A-30 | 同一原料一个编码，不同厂号/产地区分 | RawMaterialType + MaterialBatch | P0 | ✅已建 | V2 | RawMaterialType.primaryCode 唯一，MaterialBatch 含厂号/产地可共存同一 rawMaterialTypeId，无多厂号共存自动去重测试 | API断言同一 rawMaterialTypeId 下建2个不同 factoryNumber 批次均成功 |
...
| A-32 | 录入时厂号录清楚，供单发下去后生产人员选 | MaterialBatch + 领料单 | P0 | ✅已建 | V2 | MaterialBatch 含厂号字段（confirmed），领料单选批次时展示厂号，链路通，无"领料必须呈现厂号"UI 断言 | headed UI: 领料时验批次列表含厂号/产地列 |
...
| A-35 | 区分入库登记与采购原料登记/新建原料 | PurchaseOrder + MaterialBatch | P0 | ✅已建 | V2 | 采购入库与原料档案是独立实体（PurchaseOrder/MaterialBatch），链路通，无"两步"流程 E2E 断言 | headed UI: 采购→入库→原料档案全链路走通 |
...
| A-43 | 全系统设计全关联（BOM互相串联） | BOM + SP8 primaryCode | P0 | 🟡部分 | V2 | primaryCodeRef 实现了"BOM关联前3位主编码"，全链扫码弹出关联包材的"自动弹出"场景尚未有 headed E2E 断言 | headed UI: 输入包材编码验自动弹出关联 BOM |
...
| A-52 | 正常品大BOM只规定一种包材 | BomRecipe | ⚪约束项 | ✅已建 | V2 | BomRecipeItem 含 itemType，可区分主料/包材/辅料，链路通；无"大品BOM=1种包材"业务约束断言 | 真实数据：录入叮咚好食光猪蹄BOM验包材只有一种 |
...
| B-05 | 实际成本与定价偏差需及时反馈研发/销售 | CostVarianceConfig + 通知 | P1 | 🟡部分 | V2 | 偏差计算已实现（SP3），通知推送机制（研发/销售收报警）无专项测试 | headed UI: 验超支报警后研发/销售端消息提示 |
...
| B-10 | 成本核算口径=实际入库值，异常回传采购 | PurchaseOrder + 成本 | P0 | 🟡部分 | V2 | 实际入库量成本基准已实现；"改数量必须回传采购异常"（超收220/少收38的通知链）无专项断言 | API断言入库量≠PO数量时采购单标异常状态 |
...
| B-16 | 成本/价格可随时设定、随时重算 | ProductType + BomRecipe | P0 | ✅已建 | V2 | PUT /product-types/{id} 支持价格更新，BomRecipeItem 可更新 unitPrice，链路通，无"重算触发"自动化断言 | API断言修改 unitPrice 后成本分析返回新值 |
...
| B-25 | 每盒人工可按工序段拆分（前提：每工序工时×人数已采集） | 报工三阶段 + 工序成本 | P1 | 🟡部分 | V2 | 三阶段报工 PR#438 含工序工时×人数采集；按工序拆分到盒的分配算法未在 `CostVarianceServiceTest` 中断言 | 真实数据：叮咚猪舌6道工序录入后，验每道工序摊到盒的成本 |
...
| B-28 | BOM价格跟采购走，最源头由采购填 | PurchaseOrder + BomRecipe | P0 | ✅已建 | V2 | 采购入库单含 unitPrice，BomRecipeItem.unitPrice 可手动录入，链路通；"采购填价→BOM自动同步"无自动触发断言 | API断言采购入库后 BomRecipeItem 价格更新（P1实现）|
...
| B-31 | 不分算，直接引前期维护价格作成本 | BomRecipe unitPrice | P0 | ✅已建 | V2 | BomRecipeItem.unitPrice 直接引用，链路通，无"引用前期维护"vs"每次重算"对比测试 | API断言成本计算直接用 unitPrice 不重采购历史价 |
...
| B-36 | 配方/BOM由技术维护，维护后直接拉料 | BomRecipe + 领料 | P0 | ✅已建 | V2 | BomRecipe CRUD 含技术角色权限，领料单引用 BOM，链路通，无"BOM维护→拉料"自动化断言 | headed UI: 技术角色更新 BOM → 新领料单自动引用新 BOM |
| B-37 | BOM配方固定，不主动变动，只人工实时改 | BomRecipe 版本控制 | ⚪约束项 | 🟡部分 | V2 | BomRecipe 版本（BomVersion 复用自工厂，origin/main confirmed），但"人工成本独立实时修改"vs"BOM配方固定"的分层控制未有专项断言 | API断言修改 BomRecipeItem.unitPrice 不触发版本变更 |
| B-38 | 配方可后台调整 | BomRecipe CRUD | P0 | ✅已建 | V2 | BomRecipe PUT/DELETE endpoints 存在，链路通 | API断言管理员可修改 BomRecipeItem 配比 |
...
| B-41 | 配料/辅料整体固定为BOM份数 | BomRecipeItem 辅料 | P0 | ✅已建 | V2 | BomRecipeItem 含辅料行（itemType=AUXILIARY），链路通，无"辅料固定份数不分次"断言 | API断言辅料 BomRecipeItem.quantity 为固定值 |
...
| B-47 | 半成品混合加权计价（淋1吨用500kg，下批500旧+新1吨同步） | SemiFinishedInventory 加权平均 | P0 | 🟡部分 | V2 | `SemiFinishedInventory` rolling average unitCost 存在；多批次混合加权平均算法（spec §B-47）未在 `G6/G7/G8` test 中有精确数值断言 | 真实数据：两批次投入→验加权平均 unitCost = 期望值 |
| B-48 | 猪蹄焯水/熟制半成品两个价格冲突（单库+多SKU code） | SemiFinishedInventory + SKU | P0 | 🟡部分 | V2 | SemiFinishedInventory 含 productType FK 可区分不同半成品，链路通；"同库多SKU两价格不串"无专项测试 | API断言同一仓库两个不同半成品 SKU 的 unitCost 独立 |
...
| B-51 | 包菜要做进BOM（有材料） | BomRecipeItem 包材 | P0 | ✅已建 | V2 | BomRecipeItem PACKAGING 类型支持，链路通，无包菜具体 BOM 录入测试 | 真实数据：叮咚猪蹄 BOM 录入包菜配料验可保存 |
| B-52 | 包材建档唯一码→录入BOM自动关联 | 同 A-42 | P0 | 同A-42 ✅已建 | V1 | 同 A-42 | 同 A-42 |
| B-53 | 全系统全关联（BOM互相串联） | 同 A-43 | P0 | 同A-43 🟡部分 | V2 | 同 A-43 | 同 A-43 |
...
| B-58 | 工价/工时前期测算形成固定价格表 | WorkProcess.standardHourlyRate | P0 | ✅已建 | V2 | WorkProcess entity 含 standardHourlyRate（origin/main）；F006 P0+P1 PR#424 confirmed；无固定价格表 UI 专项测试 | headed UI: 工序配置页验 standardHourlyRate 可录入 |
...
| V1强 | 27 |
| V2弱 | 14 |
| V0未验证 | 6 (B-03阻塞/B-27阻塞/B-29/B-40/B-50/B-55/B-59) |
| B阻塞 | 2 (B-03/B-27) |
| N/A | 11 |
...
| G-03 | 工序数据来源中间环节省掉（研发直填） | QuotationTask 简化 | P0 | ✅已建 | V2 | QuotationTask 支持研发直填 laborPerKg（SP10），中间环节非必须，链路通；无"中间环节 optional"的约束断言 | headed UI: 验研发可不填工序明细直接提交报价 |
...
| G-05 | 建档/建项统一从一个组建立 | RdRequest/QuotationTask | P0 | ✅已建 | V2 | QuotationTask + RdRequest 实体存在（origin/main），建项由技术组统一，链路通；无"单一组建立"权限断言 | API断言研发角色可建 QuotationTask |
| G-06 | 成本评估是包价核心难点 | QuotationTask 成本计算 | ⚪约束项 | ✅已建 | N/A | 约束项，成本计算功能已实现 | N/A |
| G-07 | 成本评估方式：系统自动拉取价格 | BomRecipe.unitPrice + 采购价 | P1 | 🟡部分 | V2 | BomRecipeItem.unitPrice 可录入，链路通；"系统自动拉取采购价"无触发断言（同 B-29） | headed UI: 验录入原料时可从采购历史价中选填 |
...
| G-15 | 研发建SKU时把握紧配方（含包材） | BomRecipe + ProductType | P0 | ✅已建 | V2 | BomRecipe 含原料/辅料/包材行（BomRecipeItem.itemType），链路通；无"建SKU时强制配方完整"断言 | API断言创建 QuotationTask 时 BomRecipe 含包材行 |
...
| G-23 | 正式报价取决于与预报价差异：差异大涨价或不做 | 三价对比 decision flow | P0 | ✅已建 | V2 | `ThreePriceComparisonServiceTest`（origin/main）含差异计算，但"差异大→不做"的决策触发无 UI 断言 | headed UI: 三价差异大时验 UI 展示警告 |
...
| I-07 | 人效工单含人数，逐工序统计 | WorkProcessTask.workerCount | P0 | ✅已建 | V1 | WorkProcessTask.workerCount field（origin/main）；batch1924 10道工序人数采集 E2E（ACTIVE.md，总人工¥1703）| 真实数据：叮咚猪舌6道工序人数 × 工时 = 预期人工成本 |
| I-08 | 未来上计件，计件价格由财务统一发，系统统计数据 | WorkProcessTask 计件 | P1 | 🟡部分 | V2 | 报工采集人数/工时已实现（P0），计件价格由财务发→系统统计的"计件结算"功能链路通，无"财务发价→计件统计"专项断言 | API断言 WorkProcessTask 含 pieceworkRate 字段可配置 |
...
| I-13 | 工序达成率（标准出成率±阈值报警） | LaborEfficiencyService SP9 M4 | P1 | 🟡部分 | V2 | SP9 spec M4 含 standardYieldMin/Max 和 75%/150% alerting；`LaborEfficiencyServiceImplTest` 含出成率断言，但达成率 alerting 场景未有专项 @Test | API断言出成率低于 standardYieldMin 触发 ALERT 状态 |
