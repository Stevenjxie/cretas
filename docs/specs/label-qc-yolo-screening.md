...
每张照片最终都要人工审核，因此**VL 否决的托盘不丢弃**，而是降为低置信候选保留
（`confidence` 压到 0.05–0.25，evidence 标注"初筛可疑，视觉复核认为正常"）。

理由：丢弃等于拿"人工本来就能兜底的误报"去换"漏检风险"，方向反了。保留的代价是
每张照片多约 1.3 个候选给人工过目（母图实测误报率 7%），收益是漏检概率下降。

候选按置信降序，VL 确认的排在前面，只有初筛判定的排在后面。对用户是一个统一列表，
不区分来源。
...
### 延迟：VL 已在 prod 关闭（2026-07-29）

实测对比（同一张 10 托盘 / 7 可疑的真实照片）：

| 模式 | 延迟 | 方差 |
|---|---|---|
| VL 复核，并发 2 | 19s | — |
| VL 复核，并发 4 | 12.3–12.5s（首次 23s） | 大 |
| VL 复核，并发 8 | **28–31s**（更慢） | 大 |
| **纯 YOLO（现行）** | **2.2–2.6s** | **极小** |

**并发不是越高越好** —— 并发 8 反而比并发 4 慢一倍以上，说明 VL 服务端有限流或排队。
单次 VL 调用本身在 4–15s 之间波动，是延迟不可控的根源。

Steve 拍板关闭 VL（`LABEL_QC_VL_REVIEW=0`）：人工必审已经兜底，用 5–12 倍的延迟
和不可预测的方差换 VL 的排序作用不划算。

VL 关闭后所有可疑托盘按初筛结果输出，置信度由 `_screen_confidence` 分级：
`托盘检测置信 × (单缺 0.75 / 双缺 0.45)`。双缺打折是因为它最可能是 crop 出问题
导致两个标签同时丢失，而非真的两个都没贴。这样人工列表仍可按可信度排序。

VL 通道保留在代码里，`LABEL_QC_VL_REVIEW=1` 即可重新启用。
