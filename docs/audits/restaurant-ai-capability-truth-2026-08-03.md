...
- `RESTAURANT` 类型的老板、店长、人事从真首页进入餐饮经营指挥屏；`FACTORY` 类型人事仍进入工厂 HR 首页。
- 首页 60 秒只读自动刷新今日经营汇总与明日预测排班 FactBook，显示真实请求耗时、最后接收、最新预订事件和逐段传输状态；后台页不可见时暂停轮询。
- 数字屏的今日领料/待审批来自 Java 经营汇总，预测客流/建议与现有人数/正向缺口/置信度来自 Python 预测 FactBook；接口失败保留上一份可用结果并标记部分异常。
- 人事只读取 `restaurantHr` 范围的排班 FactBook，不请求或展示领料、损耗等运营数据；大模型节点明确显示“未调用，提问时只读 FactBook”。
- 预订来源继续逐项显示平台/模拟标记，页面不把定时刷新冒充 WebSocket，也不把模拟预订冒充平台实单。
